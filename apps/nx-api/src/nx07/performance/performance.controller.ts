...
import { Nx07PerformanceService } from './performance.service';
...
export class Nx07PerformanceController {
  ...
  @Post()
  create(@CurrentUser() user: RequestUser, @Body() dto: CreatePerformanceDto) {
    return this.svc.create(user, dto);
  }

  @Patch(':id')
  patch(@CurrentUser() user: RequestUser, @Param('id') id: string, @Body() dto: PatchPerformanceDto) {
