...
export class StockTakeService {
  ...
  private async applyStockTakePosting(
    ...
    for (const item of items) {
      const diff = new PrismaNs.Decimal(item.countedQty).sub(new PrismaNs.Decimal(item.systemQty));
      if (diff.eq(0)) {
        await tx.nx03StockTakeItem.update({
          where: { id: item.id },
          data: {
            diffQty: diff,
            diffCost: new PrismaNs.Decimal(0),
            adjustType: 'N',
            updatedBy: userId,
          },
        });
