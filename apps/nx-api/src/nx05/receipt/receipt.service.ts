...
import {
  applyCrReceiptPosted,
  reverseCrReceiptPosted,
...
import type { CreateReceiptDto, PatchReceiptDto } from './dto/receipt.dto';
...
export class ReceiptService {
  ...
  private whereList(tenantId: string, q: Nx05ListQueryDto): Prisma.Nx05PaylogWhereInput {
    const parts: Prisma.Nx05PaylogWhereInput[] = [{ tenantId, payType: 'CR' }];
  ...
  async create(user: RequestUser, dto: CreateReceiptDto) {
    const tenantId = requireTenantId(user);
    return this.prisma.$transaction(async (tx) => {
      const ar = await tx.nx05ArLedger.findFirst({
        where: { id: dto.arId, tenantId },
        select: { docNo: true, customerId: true },
      });
      ...
      const org = orgCodeFromDocNo(ar.docNo);
      const docNo = await allocNx05DocNo(tx, tenantId, 'RC', org);
  ...
  async patch(user: RequestUser, id: string, dto: PatchReceiptDto) {
    const tenantId = requireTenantId(user);
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (dto.status === PaylogStatus.POSTED) {
        await applyCrReceiptPosted(tx, { tenantId, paylogId: id, userId: user.sub });
      } else if (dto.status === PaylogStatus.VOIDED && existing.status === PaylogStatus.POSTED) {
        await reverseCrReceiptPosted(tx, { tenantId, paylogId: id, userId: user.sub });
      ...
      await this.audit.write({
        ...
        summary: `收款 ${existing.status} -> ${dto.status}`,
