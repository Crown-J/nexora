...
export class PartnerService {
    ...
    async create(body: CreatePartnerBody, actorUserId?: string): Promise<PartnerDto> {
    ...
    async update(id: string, body: UpdatePartnerBody, actorUserId?: string): Promise<PartnerDto> {
    ...
    async setActive(id: string, body: SetActiveBody, actorUserId?: string): Promise<PartnerDto> {
        const exists = await this.prisma.nx00Partner.findUnique({ where: { id } });
        if (!exists) throw new NotFoundException('Partner not found');
