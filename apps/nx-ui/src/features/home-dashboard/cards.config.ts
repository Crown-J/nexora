...
//
// 每卡顯示：現況數字（用 list endpoint 的 pagination.total 當「總筆數」）
// 點卡 → 跳對應功能頁
// KPI 類（達成率/比較類）標 isPremium = true、顯示「📌 選購套件」、本軌不接 API
// endpoint 暫無對應的標 isPending、顯示「即將上線」
//
// ⚠️ 2026-06-02 Bug 2 修正：Sub 2 commit a9d2579 漏改 endpoint、實際仍是 /nxXX/dashboard/xxx-count
// （後端根本沒這些路徑、全 404）；本次真改成既有 list endpoint + pagination.total。
...
export type HomeCardDef = {
  ...
  /** 數字後綴單位（例：'件'、'元'、'%'）*/
  unit: string;
  /** 此卡的數字 API endpoint（list endpoint 用 pagination.total）；isPending / isPremium 時 null */
...
export const HOME_CARDS: HomeCardDef[] = [
  // ─── 採購 ──────────────────────────────────────────────
  {
    ...
    category: 'purchase',
    unit: '筆',
    endpoint: '/nx02/demand?pageSize=1',
    hint: '總筆數',
  },
  {
    viewCode: 'NX02_RFQ',
  ...
  },
  {
    ...
    category: 'purchase',
    unit: '筆',
    endpoint: '/nx02/warranty-claims?pageSize=1',
    hint: '總筆數',
  },
  {
    ...
    category: 'purchase',
    unit: '料',
    endpoint: '/nx03/part-stock-setting?pageSize=1',
    hint: '已設定數',
  },
  ...
  },
  {
    ...
    category: 'inventory',
    unit: '筆',
    endpoint: '/nx03/stocktake?pageSize=1',
    hint: '總筆數',
  },
  {
    ...
    category: 'inventory',
    unit: '筆',
    endpoint: '/nx03/issue-report?pageSize=1',
    hint: '總筆數',
  },
  ...
  // ─── 銷售 ──────────────────────────────────────────────
  {
    ...
    category: 'sale',
    unit: '筆',
    endpoint: '/nx04/quote?pageSize=1',
    hint: '總筆數',
  },
  {
    viewCode: 'NX04_SO',
  ...
  },
  {
    ...
    category: 'sale',
    unit: '筆',
    endpoint: '/nx04/sales-return?pageSize=1',
    hint: '總筆數',
  },
  {
    viewCode: 'NX02_TI',
    ...
    category: 'sale',
    unit: '筆',
    endpoint: '/nx02/purchase-return?pageSize=1',
    hint: '總筆數（沿用退貨）',
  },
  {
  ...
  },
  {
    ...
    category: 'finance',
    unit: '期',
    endpoint: '/nx05/period-close?pageSize=1',
    hint: '已關帳數',
  },
];
...
/** 統計：適合放現況數字 21（17 串 endpoint + 4 isPending）/ KPI 選購套件 6 / 不放 7（主檔工具類）*/
