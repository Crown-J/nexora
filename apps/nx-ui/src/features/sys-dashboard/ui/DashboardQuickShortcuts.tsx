 ...
 * 首頁左側五鍵快捷列（與行事曆列等高；垂直 space-evenly）
 */
  ...
  return (
    <div
      className={cx(
        ...
        dimClass,
        isVertical ? 'flex w-[72px] flex-col justify-evenly' : 'flex w-full flex-row justify-evenly py-2',
        className,
      )}
      aria-label="首頁預設快捷鍵"
    >
      {DASHBOARD_QUICK_SHORTCUTS.map(({ key, label, href, Icon }) => (
        <button
          ...
          className={cx(
            'flex flex-col items-center justify-center gap-1 rounded-xl border border-border/70 bg-muted/20 px-1 py-2 text-[10px] font-medium text-foreground transition hover:border-primary/40 hover:bg-muted/45',
            !isVertical && 'min-w-0 flex-1',
          )}
        >
          <Icon className="h-5 w-5 shrink-0 text-primary" aria-hidden />
          <span className="max-w-[4.5rem] text-center leading-tight">{label}</span>
          <span className="font-mono text-[9px] text-muted-foreground">{key.toUpperCase()}</span>
        </button>
      ))}
    </div>
  );
