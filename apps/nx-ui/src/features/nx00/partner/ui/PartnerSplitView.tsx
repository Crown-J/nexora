/**
 * File: apps/nx-ui/src/features/nx00/partner/ui/PartnerSplitView.tsx
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-PARTNER-SPLIT-VIEW-001：Partner Split View（左 list 右 form）
 */
...
import { usePartnerSplit } from '@/features/nx00/partner/hooks/usePartnerSplit';
import { PartnerFormPanel } from '@/features/nx00/partner/ui/PartnerFormPanel';
import { PARTNER_FIELDS, type PartnerFieldKey } from '@/features/nx00/partner/meta/partner.fields';
import type { PartnerDto, PartnerType } from '@/features/nx00/partner/types';

type SortState = { key: PartnerFieldKey; dir: 'asc' | 'desc' } | null;

type PartnerListConfig = {
    visibleCols: PartnerFieldKey[];
    colOrder: PartnerFieldKey[];
};

function buildDefs(): Record<PartnerFieldKey, ColumnDef<PartnerFieldKey>> {
    const map = {} as Record<PartnerFieldKey, ColumnDef<PartnerFieldKey>>;
...
export function PartnerSplitView() {
    ...
    // ===== 欄位定義 =====
    const allKeys = useMemo(() => PARTNER_FIELDS.map((f) => f.key), []);
    const defaultVisible = useMemo(() => PARTNER_FIELDS.filter((f) => f.inList).map((f) => f.key), []);
    const defaultOrder = useMemo(() => PARTNER_FIELDS.map((f) => f.key), []);
    const defsByKey = useMemo(() => buildDefs(), []);

    // ===== list 設定（localStorage）=====
    const pref = useListLocalPref<PartnerListConfig>('nx00.partner.listConfig', 1, {
    ...
    const visibleFieldDefs = useMemo(() => {
        const lockedKey: PartnerFieldKey = 'code';
        const visible = pref.value?.visibleCols?.includes(lockedKey)
            ? pref.value.visibleCols
            : ([lockedKey, ...(pref.value?.visibleCols ?? [])] as PartnerFieldKey[]);

        const ordered = (pref.value?.colOrder ?? []).filter((k) => visible.includes(k));
        return PARTNER_FIELDS.filter((f) => ordered.includes(f.key)).sort((a, b) => ordered.indexOf(a.key) - ordered.indexOf(b.key));
    }, [pref.value]);
    ...
    const toggleSort = (key: PartnerFieldKey) => {
        setSort((prev) => {
    ...
    const renderCell = (row: PartnerDto, key: PartnerFieldKey) => {
        const v = (row as any)[key];

        if (key === 'partnerType') return typeText(v as PartnerType);
    ...
    const stickyLeftKeys = useMemo<PartnerFieldKey[]>(() => ['code'], []);

    return (
        <>
            <PageHeader title="交易夥伴基本資料" />
            ...
            <div className="flex h-[calc(100vh-200px)] gap-3">
                    ...
                    <div className="mb-3 flex items-center gap-2">
                        ...
                        <div className="flex flex-1 items-center gap-2">
                            <input
                                ...
                                placeholder="輸入後自動搜尋（代碼 / 名稱 / 聯絡人 / 電話）"
                                value={searchInput}
                                            ...
                                            {visibleFieldDefs.map((f) => {
                                                ...
                                                return (
                                                    <td
                                                        ...
                                                        onClick={() => vm.actions.selectPartner(r.id)}
                ...
                <div className="w-[42%]">
                    <PartnerFormPanel
                        ...
                        onCreate={vm.createPartner}
                        onUpdate={vm.updatePartner}
                    />
                </div>
            </div>
        </>
