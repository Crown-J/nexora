/**
 * File: apps/nx-ui/src/features/nx00/user/ui/UserSplitView.tsx
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-USER-SPLIT-VIEW-001：User Split View（左 list 右 form）
 *
 * Notes:
 * - 左側：list（欄位顯示選擇 / 多選 / 排序 / 自動搜尋 / 水平滾動 / 設定記憶）
 * - 右側：UserFormPanel（檢視/編輯切換）
...
import { useUserSplit } from '@/features/nx00/user/hooks/useUserSplit';
import { UserFormPanel } from '@/features/nx00/user/ui/UserFormPanel';
import { USER_FIELDS, type UserFieldKey } from '@/features/nx00/user/meta/user.fields';
import type { UserDto } from '@/features/nx00/user/types';

type SortState = { key: UserFieldKey; dir: 'asc' | 'desc' } | null;

type UserListConfig = {
    visibleCols: UserFieldKey[];
    colOrder: UserFieldKey[];
};

function buildDefs(): Record<UserFieldKey, ColumnDef<UserFieldKey>> {
    const map = {} as Record<UserFieldKey, ColumnDef<UserFieldKey>>;
    USER_FIELDS.forEach((f) => {
        map[f.key] = {
            key: f.key,
            label: f.label,
            locked: f.key === 'username', // User 最少保留 username（避免列表空）
        };
    });
    return map;
}

export function UserSplitView() {
    const vm = useUserSplit();
    ...
    // ===== 欄位定義 =====
    const allKeys = useMemo(() => USER_FIELDS.map((f) => f.key), []);
    const defaultVisible = useMemo(() => USER_FIELDS.filter((f) => f.inList).map((f) => f.key), []);
    const defaultOrder = useMemo(() => USER_FIELDS.map((f) => f.key), []);
    const defsByKey = useMemo(() => buildDefs(), []);

    // ===== list 設定（localStorage）=====
    const pref = useListLocalPref<UserListConfig>('nx00.user.listConfig', 1, {
    ...
    const visibleFieldDefs = useMemo(() => {
        const lockedKey: UserFieldKey = 'username';
        const visible = pref.value?.visibleCols?.includes(lockedKey)
            ? pref.value.visibleCols
            : ([lockedKey, ...(pref.value?.visibleCols ?? [])] as UserFieldKey[]);

        const ordered = (pref.value?.colOrder ?? []).filter((k) => visible.includes(k));

        return USER_FIELDS.filter((f) => ordered.includes(f.key)).sort((a, b) => ordered.indexOf(a.key) - ordered.indexOf(b.key));
    }, [pref.value]);
    ...
    const toggleSort = (key: UserFieldKey) => {
        setSort((prev) => {
    ...
    const renderCell = (row: UserDto, key: UserFieldKey) => {
    ...
    const stickyLeftKeys = useMemo<UserFieldKey[]>(() => ['username'], []);
                    ...
                    <div className="mb-3 flex items-center gap-2">
                        ...
                        <div className="flex flex-1 items-center gap-2">
                            <input
                                ...
                                placeholder="輸入後自動搜尋（帳號 / 顯示名稱 / Email）"
                                value={searchInput}
                                            ...
                                            {visibleFieldDefs.map((f) => {
                                                ...
                                                return (
                                                    <td
                                                        ...
                                                        onClick={() => vm.actions.selectUser(r.id)}
