/**
 * File: apps/nx-ui/src/features/nx00/role/hooks/useRoleSplit.ts
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-ROLE-SPLIT-HOOK-001：Role Split View Orchestrator（URL 驅動狀態）
 *
 * Notes:
...
import { createRole, getRole, listRole, updateRole } from '@/features/nx00/role/api/role';
import type { CreateRoleBody, RoleDto, UpdateRoleBody } from '@/features/nx00/role/types';

export function useRoleSplit() {
    ...
    useEffect(() => {
        ...
        listRole({ q: q || undefined, page, pageSize })
            .then((res) => {
                if (!alive) return;
                setItems(res.items ?? []);
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-ROLE-SPLIT-HOOK-001-F02
     * 說明：
     * - createOne：新增角色
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-ROLE-SPLIT-HOOK-001-F03
     * 說明：
     * - updateOne：更新角色
