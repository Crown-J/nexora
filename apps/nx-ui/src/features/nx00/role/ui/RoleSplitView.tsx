/**
 * File: apps/nx-ui/src/features/nx00/role/ui/RoleSplitView.tsx
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-ROLE-SPLIT-VIEW-001：Role Split View（左 list 右 form）
 *
 * Notes:
 * - 左側：list（欄位顯示選擇 / 多選 / 排序 / 自動搜尋 / 水平滾動 / 設定記憶）
 * - 右側：RoleFormPanel（檢視/編輯切換）
 * - List+Form 共用 UI 已抽到 shared/ui/listform
 * - 搜尋 debounce / 多選 selection 已抽到 shared/hooks
...
import { useRoleSplit } from '@/features/nx00/role/hooks/useRoleSplit';
import { RoleFormPanel } from '@/features/nx00/role/ui/RoleFormPanel';
import { ROLE_FIELDS, type RoleFieldKey } from '@/features/nx00/role/meta/role.fields';
import type { RoleDto } from '@/features/nx00/role/types';

type SortState = { key: RoleFieldKey; dir: 'asc' | 'desc' } | null;

type RoleListConfig = {
    visibleCols: RoleFieldKey[];
    colOrder: RoleFieldKey[];
};

function buildDefs(): Record<RoleFieldKey, ColumnDef<RoleFieldKey>> {
    const map = {} as Record<RoleFieldKey, ColumnDef<RoleFieldKey>>;
...
export function RoleSplitView() {
    const vm = useRoleSplit();
    ...
    // ===== 欄位定義 =====
    const allKeys = useMemo(() => ROLE_FIELDS.map((f) => f.key), []);
    const defaultVisible = useMemo(() => ROLE_FIELDS.filter((f) => f.inList).map((f) => f.key), []);
    const defaultOrder = useMemo(() => ROLE_FIELDS.map((f) => f.key), []);
    const defsByKey = useMemo(() => buildDefs(), []);

    // ===== list 設定（localStorage）=====
    const pref = useListLocalPref<RoleListConfig>('nx00.role.listConfig', 1, {
    ...
    const visibleFieldDefs = useMemo(() => {
        const lockedKey: RoleFieldKey = 'code';
        const visible = pref.value?.visibleCols?.includes(lockedKey)
            ? pref.value.visibleCols
            : ([lockedKey, ...(pref.value?.visibleCols ?? [])] as RoleFieldKey[]);

        const ordered = (pref.value?.colOrder ?? []).filter((k) => visible.includes(k));

        return ROLE_FIELDS.filter((f) => ordered.includes(f.key)).sort((a, b) => ordered.indexOf(a.key) - ordered.indexOf(b.key));
    }, [pref.value]);

    // ===== 多選 checkbox（抽到 shared hook）=====
    ...
    const toggleSort = (key: RoleFieldKey) => {
        setSort((prev) => {
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-ROLE-SPLIT-VIEW-001-F01
     * 說明：
     * - renderCell：列表欄位統一渲染（bool/date/null）
     */
    const renderCell = (row: RoleDto, key: RoleFieldKey) => {
    ...
    const stickyLeftKeys = useMemo<RoleFieldKey[]>(() => ['code'], []);
