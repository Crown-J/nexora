/**
 * File: apps/nx-ui/src/features/nx00/roles/hooks/useRolesSplit.ts
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-ROLES-SPLIT-HOOK-001：Roles Split View Orchestrator（URL 驅動狀態）
 *
 * Notes:
...
import { createRole, getRole, listRoles, updateRole } from '@/features/nx00/roles/api/roles';
...
export function useRolesSplit() {
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-ROLES-SPLIT-HOOK-001-F01
     * 說明：
    ...
    useEffect(() => {
        ...
        listRoles({ q: q || undefined, page, pageSize })
            .then((res) => {
                if (!alive) return;
                setItems(res.items ?? []);
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-ROLES-SPLIT-HOOK-001-F02
     * 說明：
     * - createOne：新增角色
     * - 成功後：若在第 1 頁則 optimistic prepend；並切到 edit 模式（id=created.id）
     */
    const createOne = useCallback(
        async (body: CreateRoleBody) => {
            setSaving(true);
            setSaveError(null);
            try {
                const created = await createRole(body);

                // ✅ 同步更新 list：只在 page=1 時 prepend，避免其他頁面顯示怪異
                if (page === 1) {
                    setItems((prev) => {
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-ROLES-SPLIT-HOOK-001-F03
     * 說明：
     * - updateOne：更新角色
     * - 成功後：detail 與 list 同步更新（避免重整）
     */
    const updateOne = useCallback(async (id: string, body: UpdateRoleBody) => {
        setSaving(true);
        setSaveError(null);
        try {
    ...
    const actions = useMemo(
        () => ({
            ...
            selectRole: (id: string) => setQuery({ id, mode: null }),
