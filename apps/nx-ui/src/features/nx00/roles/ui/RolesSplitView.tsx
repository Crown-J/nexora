/**
 * File: apps/nx-ui/src/features/nx00/roles/ui/RolesSplitView.tsx
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-ROLES-SPLIT-VIEW-001：Roles Split View（左 list 右 form）
 *
 * Notes:
 * - 左側：list（欄位顯示選擇 / 多選 / 排序 / 自動搜尋 / 水平滾動 / 設定記憶）
 * - 右側：RoleFormPanel（檢視/編輯切換）
...
import { useRolesSplit } from '@/features/nx00/roles/hooks/useRolesSplit';
import { RoleFormPanel } from '@/features/nx00/roles/ui/RoleFormPanel';
import { ROLES_FIELDS, type RolesFieldKey } from '@/features/nx00/roles/meta/roles.fields';
import type { RoleDto } from '@/features/nx00/roles/types';

type SortState = { key: RolesFieldKey; dir: 'asc' | 'desc' } | null;

type RolesListConfig = {
    visibleCols: RolesFieldKey[];
    colOrder: RolesFieldKey[];
};

function buildDefs(): Record<RolesFieldKey, ColumnDef<RolesFieldKey>> {
    const map = {} as Record<RolesFieldKey, ColumnDef<RolesFieldKey>>;
    ROLES_FIELDS.forEach((f) => {
        map[f.key] = {
            key: f.key,
            label: f.label,
            locked: f.key === 'code', // Role 最少要保留 code（避免列表空）
        };
    });
    return map;
}

export function RolesSplitView() {
    const vm = useRolesSplit();
    ...
    // ===== 欄位定義 =====
    const allKeys = useMemo(() => ROLES_FIELDS.map((f) => f.key), []);
    const defaultVisible = useMemo(() => ROLES_FIELDS.filter((f) => f.inList).map((f) => f.key), []);
    const defaultOrder = useMemo(() => ROLES_FIELDS.map((f) => f.key), []);
    const defsByKey = useMemo(() => buildDefs(), []);

    // ===== list 設定（localStorage）=====
    const pref = useListLocalPref<RolesListConfig>('nx00.roles.listConfig', 1, {
    ...
    const visibleFieldDefs = useMemo(() => {
        const lockedKey: RolesFieldKey = 'code';
        const visible = pref.value?.visibleCols?.includes(lockedKey)
            ? pref.value.visibleCols
            : ([lockedKey, ...(pref.value?.visibleCols ?? [])] as RolesFieldKey[]);

        const ordered = (pref.value?.colOrder ?? []).filter((k) => visible.includes(k));

        return ROLES_FIELDS.filter((f) => ordered.includes(f.key)).sort((a, b) => ordered.indexOf(a.key) - ordered.indexOf(b.key));
    }, [pref.value]);
    ...
    const toggleSort = (key: RolesFieldKey) => {
        setSort((prev) => {
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-ROLES-SPLIT-VIEW-001-F01
     * 說明：
     * - renderCell：列表欄位統一渲染（bool/date/null）
     */
    const renderCell = (row: RoleDto, key: RolesFieldKey) => {
    ...
    const stickyLeftKeys = useMemo<RolesFieldKey[]>(() => ['code'], []);

    return (
        <>
            <PageHeader title="職位基本資料" />
            ...
            <div className="flex h-[calc(100vh-200px)] gap-3">
                ...
                <div className="w-[42%]">
                    <RoleFormPanel
                        ...
                        onCreate={vm.createRole}
                        onUpdate={vm.updateRole}
                    />
                </div>
            </div>
        </>
