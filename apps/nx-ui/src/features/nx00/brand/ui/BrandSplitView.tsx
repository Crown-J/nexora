/**
 * File: apps/nx-ui/src/features/nx00/brand/ui/BrandSplitView.tsx
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-BRAND-SPLIT-VIEW-001：Brand Split View（左 list 右 form）
 */
...
import { useBrandSplit } from '@/features/nx00/brand/hooks/useBrandSplit';
import { BrandFormPanel } from '@/features/nx00/brand/ui/BrandFormPanel';
import { BRAND_FIELDS, type BrandFieldKey } from '@/features/nx00/brand/meta/brand.fields';
import type { BrandDto } from '@/features/nx00/brand/types';

type SortState = { key: BrandFieldKey; dir: 'asc' | 'desc' } | null;

type BrandListConfig = {
    visibleCols: BrandFieldKey[];
    colOrder: BrandFieldKey[];
};

function buildDefs(): Record<BrandFieldKey, ColumnDef<BrandFieldKey>> {
    const map = {} as Record<BrandFieldKey, ColumnDef<BrandFieldKey>>;
...
export function BrandSplitView() {
    const vm = useBrandSplit();
    ...
    // ===== 欄位定義 =====
    const allKeys = useMemo(() => BRAND_FIELDS.map((f) => f.key), []);
    const defaultVisible = useMemo(() => BRAND_FIELDS.filter((f) => f.inList).map((f) => f.key), []);
    const defaultOrder = useMemo(() => BRAND_FIELDS.map((f) => f.key), []);
    const defsByKey = useMemo(() => buildDefs(), []);

    // ===== list 設定（localStorage）=====
    const pref = useListLocalPref<BrandListConfig>('nx00.brand.listConfig', 1, {
    ...
    const visibleFieldDefs = useMemo(() => {
        const lockedKey: BrandFieldKey = 'code';
        const visible = pref.value?.visibleCols?.includes(lockedKey)
            ? pref.value.visibleCols
            : ([lockedKey, ...(pref.value?.visibleCols ?? [])] as BrandFieldKey[]);

        const ordered = (pref.value?.colOrder ?? []).filter((k) => visible.includes(k));
        return BRAND_FIELDS.filter((f) => ordered.includes(f.key)).sort((a, b) => ordered.indexOf(a.key) - ordered.indexOf(b.key));
    }, [pref.value]);
    ...
    const toggleSort = (key: BrandFieldKey) => {
        setSort((prev) => {
    ...
    const renderCell = (row: BrandDto, key: BrandFieldKey) => {
    ...
    const stickyLeftKeys = useMemo<BrandFieldKey[]>(() => ['code'], []);
            ...
            <div className="flex h-[calc(100vh-200px)] gap-3">
                                            ...
                                            {visibleFieldDefs.map((f) => {
                                                ...
                                                return (
                                                    <td
                                                        ...
                                                        onClick={() => vm.actions.selectBrand(r.id)}
                ...
                <div className="w-[42%]">
                    <BrandFormPanel
                        ...
                        onCreate={vm.createBrand}
                        onUpdate={vm.updateBrand}
                    />
                </div>
            </div>
        </>
