/**
 * File: apps/nx-ui/src/features/nx00/permissions/hooks/usePermissionsSplit.ts
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-PERM-SPLIT-HOOK-001：Permissions Split View Orchestrator（URL 驅動狀態）
 *
 * Notes:
...
import { createPermission, getPermission, listPermissions, updatePermission } from '@/features/nx00/permissions/api/permissions';
...
export function usePermissionsSplit() {
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-PERM-SPLIT-HOOK-001-F01
     * 說明：
    ...
    useEffect(() => {
        ...
        listPermissions({ q: q || undefined, page, pageSize })
            .then((res) => {
                if (!alive) return;
                setItems(res.items ?? []);
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-PERM-SPLIT-HOOK-001-F02
     * 說明：
     * - createOne：新增 permission
     * - 成功後：若在第 1 頁則 optimistic prepend；並切到 edit 模式（id=created.id）
     */
    const createOne = useCallback(
        async (body: CreatePermissionBody) => {
            setSaving(true);
            setSaveError(null);
            try {
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-PERM-SPLIT-HOOK-001-F03
     * 說明：
     * - updateOne：更新 permission
     * - 成功後：detail 與 list 同步更新（避免重整）
     */
    const updateOne = useCallback(async (id: string, body: UpdatePermissionBody) => {
        setSaving(true);
        setSaveError(null);
        try {
    ...
    const actions = useMemo(
        () => ({
            ...
            selectPermission: (id: string) => setQuery({ id, mode: null }),
