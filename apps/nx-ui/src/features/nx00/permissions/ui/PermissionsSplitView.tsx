/**
 * File: apps/nx-ui/src/features/nx00/permissions/ui/PermissionsSplitView.tsx
 * Project: NEXORA (Monorepo)
 *
 * Purpose:
 * - NX00-UI-NX00-PERM-SPLIT-VIEW-001：Permissions Split View（左 list 右 form）
 *
 * Notes:
 * - 左側：list（欄位顯示選擇 / 多選 / 排序 / 自動搜尋 / 水平滾動 / 設定記憶）
 * - 右側：PermissionFormPanel（檢視/編輯切換）
 * - List+Form 共用 UI 已抽到 shared/ui/listform
 * - className 組合一律使用 cx（依專案規範）
 */
...
import { usePermissionsSplit } from '@/features/nx00/permissions/hooks/usePermissionsSplit';
import { PermissionFormPanel } from '@/features/nx00/permissions/ui/PermissionFormPanel';
import { PERMISSIONS_FIELDS, type PermissionsFieldKey } from '@/features/nx00/permissions/meta/permissions.fields';
import type { PermissionDto } from '@/features/nx00/permissions/types';

type SortState = { key: PermissionsFieldKey; dir: 'asc' | 'desc' } | null;

type PermissionsListConfig = {
    visibleCols: PermissionsFieldKey[];
    colOrder: PermissionsFieldKey[];
};

function buildDefs(): Record<PermissionsFieldKey, ColumnDef<PermissionsFieldKey>> {
    const map = {} as Record<PermissionsFieldKey, ColumnDef<PermissionsFieldKey>>;
    PERMISSIONS_FIELDS.forEach((f) => {
        map[f.key] = {
            key: f.key,
            label: f.label,
            locked: f.key === 'code',
...
export function PermissionsSplitView() {
    const vm = usePermissionsSplit();
    ...
    // ===== 欄位定義 =====
    const allKeys = useMemo(() => PERMISSIONS_FIELDS.map((f) => f.key), []);
    const defaultVisible = useMemo(() => PERMISSIONS_FIELDS.filter((f) => f.inList).map((f) => f.key), []);
    const defaultOrder = useMemo(() => PERMISSIONS_FIELDS.map((f) => f.key), []);
    const defsByKey = useMemo(() => buildDefs(), []);

    // ===== list 設定（localStorage）=====
    const pref = useListLocalPref<PermissionsListConfig>('nx00.permissions.listConfig', 1, {
    ...
    const visibleFieldDefs = useMemo(() => {
        const lockedKey: PermissionsFieldKey = 'code';
        const visible = pref.value?.visibleCols?.includes(lockedKey)
            ? pref.value.visibleCols
            : ([lockedKey, ...(pref.value?.visibleCols ?? [])] as PermissionsFieldKey[]);

        const ordered = (pref.value?.colOrder ?? []).filter((k) => visible.includes(k));

        return PERMISSIONS_FIELDS.filter((f) => ordered.includes(f.key)).sort(
    ...
    const toggleSort = (key: PermissionsFieldKey) => {
        setSort((prev) => {
    ...
    /**
     * @FUNCTION_CODE NX00-UI-NX00-PERM-SPLIT-VIEW-001-F01
     * 說明：
     * - renderCell：列表欄位統一渲染（bool/date/null/number）
     */
    const renderCell = (row: PermissionDto, key: PermissionsFieldKey) => {
    ...
    // ===== sticky-left 欄位（checkbox + code）=====
    const stickyLeftKeys = useMemo<PermissionsFieldKey[]>(() => ['code'], []);

    return (
        <>
            <PageHeader title="權限基本資料" />
            ...
            <div className="flex h-[calc(100vh-200px)] gap-3">
                    ...
                    <div className="mb-3 flex items-center gap-2">
                        ...
                        <div className="flex flex-1 items-center gap-2">
                            <input
                                ...
                                placeholder="輸入後自動搜尋（代碼 / 名稱 / 模組代碼 / 動作）"
                                value={searchInput}
                                            ...
                                            {visibleFieldDefs.map((f) => {
                                                ...
                                                return (
                                                    <td
                                                        ...
                                                        onClick={() => vm.actions.selectPermission(r.id)}
                ...
                <div className="w-[42%]">
                    <PermissionFormPanel
                        ...
                        onCreate={vm.createPermission}
                        onUpdate={vm.updatePermission}
                    />
                </div>
            </div>
        </>
