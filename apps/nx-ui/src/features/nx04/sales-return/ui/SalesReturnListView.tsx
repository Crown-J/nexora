...
import { createSr, listSr } from '@data/endpoints/nx04/sales-return/api/sales-return';
import type { CreateSrPayload, Sr, SrStatus } from '@data/types/nx04/sales-return';
import { SR_STATUS_BADGE_CLASS, SR_STATUS_LABEL, SR_STATUSES } from '@data/types/nx04/sales-return';
...
export function SalesReturnListView() {
  ...
  return (
    <div className="w-full min-w-0 space-y-6 p-6">
      ...
      {showNew ? (
        <QuickCreateForm
          onCreated={(id) => router.push(`/dashboard/sale/return/${id}`)}
              ...
              {rows.map((r) => (
                <tr key={r.id} className="border-t hover:bg-muted/50">
                  ...
                  <td className="px-3 py-2 text-right">
                    <Link
                      href={`/dashboard/sale/return/${r.id}`}
