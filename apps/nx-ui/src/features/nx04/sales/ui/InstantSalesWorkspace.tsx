...
/*
 * ⚠️ 2026-08-02：型別與規則搬到 features/nx04/sales/so-draft.ts，本檔改用 import。
 *    ⛔ 不是複製一份過去——新的一頁式「建立銷貨單」與這個舊浮層工作站是同一件事的兩個外殼，
 *       規則有兩份的話改了一邊沒改另一邊，帳期會算出兩種答案、客戶會收到兩種訊息格式。
 *    ⭐ 這是既有前例：報價訊息當年就是為了「避免兩邊 drift」抽成 quote-message.ts。
 *    ⛔ 本檔的行為完全沒有改變（純搬移）。
 */
import {
  autoAllocate,
  buildSalesMessage as buildMessage,
  DEFAULT_SALES_MSG_OPTS as DEFAULT_MSG_OPTS,
  ...
  PAYMENT_TERMS,
  paymentTermLabel,
  SALES_MSG_OPT_DEFS as MSG_OPT_DEFS,
  SALES_MSG_OPTS_KEY as MSG_OPTS_KEY,
  shipAddressOneLine,
  taxRateOf,
  type Allocation,
  type AllocSource,
  ...
  type OrderResult,
  type SalesLine,
  type SalesMsgOpts as MsgOpts,
} from '@/features/nx04/sales/so-draft';

// 舊的對外型別出口保留（本檔原本 export 過這三個型別，⛔ 不確定有沒有別處在用就別拿掉）
export type { AllocSource, Allocation, SalesLine };
