...
// 執行長 2026-07-11：事件帶 items 陣列（右欄 Space 標記多顆）→ 改開 BatchInstantQuoteDialog 批次報價
'use client';

import { useEffect, useState } from 'react';

import { BatchInstantQuoteDialog, type BatchQuoteItem } from './BatchInstantQuoteDialog';
import { InstantQuoteDialog } from './InstantQuoteDialog';

type Item = { partId: string; code?: string; name?: string };
type Detail = Item & { items?: Item[] };
...
export function GlobalInstantQuote() {
  ...
  if (target.items && target.items.length > 1) {
    const items: BatchQuoteItem[] = target.items
      .filter((i) => typeof i.partId === 'string' && i.partId)
      .map((i) => ({ partId: i.partId, code: i.code ?? '', name: i.name ?? '' }));
    return <BatchInstantQuoteDialog items={items} onClose={() => setTarget(null)} />;
  }
  return (
    <InstantQuoteDialog
