...
function fmtCost(n: number): string {
  return new Intl.NumberFormat('zh-TW', { minimumFractionDigits: 2, maximumFractionDigits: 4 }).format(n);
}

export function PrNewForm() {
  ...
  const submit = async () => {
    ...
    for (const it of rrDetail.items) {
      ...
      if (q > it.qty) {
        setError(`退貨數量不可超過原進貨量：${it.partNo}（最多 ${it.qty}）`);
        return;
      }
      ...
      {step === 1 ? (
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            退貨單須由已過帳的進貨單帶入；倉庫與供應商將於下一步自動帶入，無須手動建立空白單。
          </p>
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              <thead className="border-b bg-muted/30 text-xs text-muted-foreground">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right">單價（原進貨）</th>
                ...
                {rrDetail.items.map((it) => {
                  ...
                  return (
                    <tr key={it.id} className="border-b border-border/50">
                      ...
                      <td className="px-3 py-2 text-right tabular-nums text-muted-foreground">{fmtCost(it.unitCost)}</td>
