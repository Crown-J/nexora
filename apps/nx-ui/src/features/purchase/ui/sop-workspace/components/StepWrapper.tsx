// apps/nx-ui/src/features/purchase/ui/sop-workspace/components/StepWrapper.tsx
/**
 * 每個 Step 統一的版面殼：
 * - 內容區（可滾動、上方留給 ProgressHeader、下方留給固定按鈕列）
 * - 底部 fixed 按鈕列：上一步 + 主 CTA（下一步 / 送出審核 / 模擬等）
 *
 * 使用：
 *   <StepWrapper canProceed={...} onBack={...} onNext={...} nextLabel="送出審核">
 *     <內容 JSX />
 *   </StepWrapper>
 */
...
export type StepWrapperProps = {
  children: ReactNode;
  canProceed: boolean;
  /** 有上一步按鈕時顯示；STEP 1 可不給 */
  onBack?: () => void;
  onNext: () => void;
  nextLabel?: string;
  /** 主 CTA 的視覺模式：primary（金色）/ positive（綠色完成）/ neutral（白灰） */
  tone?: 'primary' | 'positive' | 'neutral';
  /** 覆寫 disabled hint text（CTA 無法按時，隱隱提示） */
  disabledHint?: string;
};
