/**
 * File: apps/nx-ui/src/features/shared/master/role-view/api/role-view.ts
...
export async function listRoleView(params: ListRoleViewParams): Promise<PagedResult<RoleViewDto>> {
    ...
    await assertOk(res, 'nxui_master_role_view_list_001');
...
export async function grantRoleView(body: GrantRoleViewBody): Promise<RoleViewDto> {
    ...
    await assertOk(res, 'nxui_master_role_view_grant_001');
...
export async function updateRoleViewPerms(id: string, body: UpdateRoleViewPermsBody): Promise<RoleViewDto> {
    ...
    await assertOk(res, 'nxui_master_role_view_update_perms_001');
    return (await res.json()) as RoleViewDto;
}
...
export async function revokeRoleView(id: string, body: Record<string, any> = {}): Promise<RoleViewDto> {
    ...
    await assertOk(res, 'nxui_master_role_view_revoke_001');
    return (await res.json()) as RoleViewDto;
}
...
export async function setRoleViewActive(id: string, isActive: boolean): Promise<RoleViewDto> {
    ...
    await assertOk(res, 'nxui_master_role_view_set_active_001');
    return (await res.json()) as RoleViewDto;
}
...
export async function listView(params: ListViewParams = {}): Promise<PagedResult<ViewDto>> {
    ...
    await assertOk(res, 'nxui_master_view_list_001');
...
export async function getRoleViewByRoleId(roleId: string): Promise<RoleViewDto[]> {
    ...
    await assertOk(res, 'nxui_master_role_view_get_by_role_001');
    return (await res.json()) as RoleViewDto[];
}
...
export async function upsertRoleView(roleId: string, items: UpsertRoleViewItem[]): Promise<RoleViewDto[]> {
    ...
    await assertOk(res, 'nxui_master_role_view_upsert_by_role_001');
    return (await res.json()) as RoleViewDto[];
}