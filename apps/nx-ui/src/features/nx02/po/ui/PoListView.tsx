...
export function PoListView() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await listPo({ page, pageSize: 20, status: status || undefined });
      setRows(r.data);
      setTotal(r.total);
    } catch (e) {
    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <h1 className="text-xl font-semibold">採購單</h1>
        </div>
        <Link href="/dashboard/purchase/po/new" className="rounded-lg bg-primary px-3 py-2 text-sm font-medium text-primary-foreground">
          + 新增採購
        </Link>
      </header>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-border/50 hover:bg-muted/20">
                <td className="px-3 py-2">
                  <Link href={`/dashboard/purchase/po/${encodeURIComponent(r.id)}`} className="font-mono text-primary hover:underline">
                    {r.docNo}
                  </Link>
                </td>
                <td className="px-3 py-2 tabular-nums text-muted-foreground">{r.poDate}</td>
                <td className="px-3 py-2">{r.supplierName}</td>
                <td className="px-3 py-2 tabular-nums">{r.itemCount}</td>
                <td className="px-3 py-2">{poStatusLabel(r.status)}</td>
