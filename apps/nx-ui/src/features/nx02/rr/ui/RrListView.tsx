...
import { listRr } from '@data/endpoints/nx02/api/rr';
import type { RrListRow } from '@data/types/nx02';
import { rrStatusLabel } from '../../shared/nx01-labels';

export function RrListView() {
  const [rows, setRows] = useState<RrListRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await listRr({ page, pageSize: 20, status: status || undefined });
      setRows(r.data);
      setTotal(r.total);
    } catch (e) {
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-border/50 hover:bg-muted/20">
                ...
                <td className="px-3 py-2">{rrStatusLabel(r.status)}</td>
