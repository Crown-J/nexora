...
export function BaseUserWarehouseView() {
  ...
  const [membersByWh, setMembersByWh] = useState<Record<string, WhMemberRow[]>>({});
  ...
  const whListKbRef = useRef<HTMLDivElement | null>(null);
    ...
    try {
      ...
      setPickerUsers(
        ur.items.map((u) => {
          ...
          const legacy = formatWarehouseLabel(u.warehouseCode, u.warehouseName);
          return {
            id: u.id,
            username: u.username,
            displayName: u.displayName,
            jobTitle: u.jobTitle ?? '—',
            warehouseLabel: summary || (legacy !== '\u2014' ? legacy : '\u2014'),
            email: u.email ?? '',
            phone: u.phone ?? '',
            isActive: u.isActive,
      ...
      setSelectedId((cur) => {
        if (cur && rows.some((w) => w.id === cur)) return cur;
  ...
  const filteredWarehouses = useMemo(() => {
    ...
    return warehouses.filter((w) => {
      const blob = `${w.code} ${w.name}`.toLowerCase();
  ...
  const currentMembers = useMemo(() => {
    ...
    return membersByWh[selectedId] ?? [];
  }, [membersByWh, selectedId]);
  ...
  const onSelectWh = useCallback((w: WarehouseDto) => {
    setSelectedId(w.id);
    setUserQuery('');
    setUserOpen(false);
    setMemberSearch('');
  ...
  const assignUser = async (userId: string) => {
    ...
    setLoadError(null);
    try {
      await assignUserWarehouse({ userId, warehouseId: selectedId });
  ...
  const removeMember = async (memberId: string) => {
    ...
    setLoadError(null);
    try {
      await revokeUserWarehouse(memberId);
      await loadMembers(selectedId);
    } catch (e) {
  ...
  const focusWhListKb = useCallback(() => {
    requestAnimationFrame(() => whListKbRef.current?.focus({ preventScroll: true }));
  }, []);

  const onWhListKeyDownCapture = useCallback(
    (e: React.KeyboardEvent) => {
      if (filteredWarehouses.length === 0) return;
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        e.stopPropagation();
        const idx = filteredWarehouses.findIndex((w) => w.id === selectedId);
        const pos = idx < 0 ? -1 : idx;
        const ni = Math.min(filteredWarehouses.length - 1, pos + 1);
        const next = filteredWarehouses[ni];
        if (next) onSelectWh(next);
        return;
      }
      if (e.key === 'ArrowUp') {
        e.preventDefault();
    ...
    [filteredWarehouses, selectedId, pathname, router, onSelectWh],
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(280px,360px)_minmax(0,1fr)] lg:items-stretch">
        <section className="glass-card flex max-h-[min(720px,calc(100vh-220px))] min-h-[420px] flex-col rounded-2xl border border-border/80 p-4 shadow-sm">
          <div className="mb-3 flex flex-col gap-2">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-xs tracking-[0.35em] text-muted-foreground">WAREHOUSES</p>
                <h2 className="mt-1 text-sm font-semibold text-foreground">據點列表</h2>
              </div>
            </div>
          ...
          <ScrollArea className="min-h-0 flex-1 pr-2">
            <div
              ...
              tabIndex={0}
              role="listbox"
              aria-label="據點列表，方向鍵選擇，Enter 進入右側使用者搜尋"
              ...
              onKeyDownCapture={onWhListKeyDownCapture}
            >
        ...
        <section className="glass-card flex min-h-[min(720px,calc(100vh-220px))] min-h-0 flex-col rounded-2xl border border-border/80 p-4 shadow-sm">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {selectedWh ? (
                <span className="text-foreground/90">
                  {selectedWh.name} <span className="text-muted-foreground">（{selectedWh.code}）</span>
                </span>
              ) : (
                '請從左側選擇據點。'
              )}
          ...
          {!selectedId ? (
            <p className="text-sm text-muted-foreground">尚無可選據點。</p>
          ) : (
            <>
              <div className="mb-3">
                <LookupAutocomplete<BaseUserRow>
                  ...
                  onEscapePanelAlreadyClosed={focusWhListKb}
              ...
              <ScrollArea className="min-h-0 flex-1 pr-2">
                <div
                  ...
                  tabIndex={0}
                  role="region"
                  aria-label="已匯入成員，方向鍵選擇，Delete 移除，Escape 回到據點列表"
                  className={cn(
                    ...
                    <>
                      ...
                      <ul className="m-0 list-none divide-y divide-border/40 p-0">
                        {memberRowsForDisplay.map((row, idx) => {
                          ...
                          return (
                            <li key={row.id} data-uw-member-row>
                              <div
                                className={cn(
