...
const ROWS: readonly AccountRow[] = [
  ...
  { code: '2142',  name: '其他應付款',               level: 3, parent: '21',   postable: true,  cf: 'O',  dept: false, partner: true,  scope: 'PARTNER', tax: null,   section: null,          active: true,  remark: '🆕 C 階段 2026-08-01 執行長拍板新增：**我們欠客戶／往來對象的錢**，主要來源是銷貨退回選「退現金」。⭐ 為什麼不塞 2141 預收貨款：那一格講的是「客戶先付的訂金」，跟「我們該退還沒退的錢」是兩件事——混在一格，將來要拆通常是在最不方便拆的時候（銀行問流動負債明細那種時候）。⛔ 也不可以記成沖減 1111 應收帳款：那會讓帳面顯示兩不相欠，但真實狀況是**方向相反**（我們欠客戶），退款義務就此從報表上消失。' },
  ...
  { code: '3103',  name: '期初承接權益',              level: 3, parent: '31',   postable: true,  cf: 'N',  dept: false, partner: false, scope: 'PARTNER', tax: null,   section: null,          active: true,  remark: '🆕 C 階段 2026-08-01 執行長拍板新增：**開帳時把既有資產帶進來的對方科目**。恆迎 2.32 億的庫存是承接進來的快照、背後沒有任何營運事件，總得有個地方承接。⛔ 不可以塞 3201 累積盈餘——那個數字的意思是「過去營運賺了多少」，塞進去等於讓它說謊。⭐ 獨立一格的好處是這筆錢永遠看得見：任何人打開資產負債表都能分辨「哪些是開帳帶進來的、哪些是自己賺的」。⚠ 一次性，開帳後不應再有新增。' },
