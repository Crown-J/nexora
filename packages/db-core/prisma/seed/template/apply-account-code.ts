...
const ROWS: readonly AccountRow[] = [
  ...
  { code: '1151',  name: '短期投資',                level: 3, parent: '11',   postable: true,  cf: 'I',  dept: false, partner: false, scope: 'PARTNER', tax: null,   section: null,          active: true,  remark: '🆕 v2 · 循環決策 1️⃣ 的配套。⚠ 投資循環 v1 砍掉，但照策略骨架 §4「範圍砍掉 ≠ 架構焊死」：**科目位要留，總帳脊椎不得假設「不會有投資類分錄」。**⛔ 狀態為預留，不可記帳；要啟用須執行長核准並補會計政策。' },
  ...
  { code: '16',    name: '長期投資',                level: 2, parent: '1',    postable: false, cf: 'N',  dept: false, partner: false, scope: 'PARTNER', tax: null,   section: null,          active: true,  remark: '🆕 v2 · 中類。同上，只留位不啟用。' },
  { code: '1601',  name: '長期股權投資',              level: 3, parent: '16',   postable: true,  cf: 'I',  dept: false, partner: true,  scope: 'PARTNER', tax: null,   section: null,          active: true,  remark: '🆕 v2 · 循環決策 1️⃣ 的配套。🔴 恆迎三十年沒有投資部位——錢全部拿去買貨了（庫存 2.9 億／其中超額 1.13 億）。亞羅 1-a 現金最低點 −429 萬，連閒置資金都不存在。⭐ 但「評估過所以不設」與「沒想過」是兩回事，這一列就是留痕。' },
  ...
  { code: '1901',  name: '開辦費',                 level: 3, parent: '19',   postable: true,  cf: 'I',  dept: false, partner: false, scope: 'PARTNER', tax: 'P5',   section: null,          active: false, remark: '⚠ A 階段停用（不刪，留痕待記帳士確認）：現行商業會計處理準則已刪除開辦費的遞延資產屬性，開辦費用應於發生時列費用。亞羅待補清單第 72 項。原說明：設立登記、資本額簽證、規費｜🆕 v2 ·『回頭要補的』#72：**這個科目不該存在**——現行商業會計處理準則已刪除開辦費的遞延資產屬性，開辦費用應於發生時列費用。⚠ 先停用不刪（歷史欄位一刪就查不回來），待記帳士確認後移除。⭐ 這是資訊循環「定期整理淘汰」在亞羅自己身上抓到的**第一筆**，而亞羅的科目表才建了四天。' },
