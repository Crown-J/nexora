...
const LINES: readonly LineRow[] = [
  ...
  { rule: 'EQ-IN',      no:  3, dc: 'C', acc: '3102',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '金額基礎（原文）：超過面額部分。🆕 v2 ·『回頭要補的』#53 ＋ #48。🔴 `3102 資本公積` 是 v2 才補的科目——**沒有它，超過面額的出資無處可放。**⚠ 有限公司通常按出資額計股本、不必然有溢價，但股東分次增資、換算比例不同時就會出現差額（記帳士確認）。', remark: '金額基礎（原文）：超過面額部分。🆕 v2 ·『回頭要補的』#53 ＋ #48。🔴 `3102 資本公積` 是 v2 才補的科目——**沒有它，超過面額的出資無處可放。**⚠ 有限公司通常按出資額計股本、不必然有溢價，但股東分次增資、換算比例不同時就會出現差額（記帳士確認）。' },
  ...
  { rule: 'LN-DRAW',    no:  1, dc: 'D', acc: '1102',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '金額基礎（原文）：動用金額。銀行撥款進帳',                  remark: '金額基礎（原文）：動用金額。銀行撥款進帳' },
  { rule: 'LN-DRAW',    no:  2, dc: 'C', acc: '2151',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '金額基礎（原文）：動用金額。🆕 v2 ·『回頭要補的』#53。⚠ 一年內到期走 `2151 短期借款`、超過一年走 `2501 長期借款`；**綜合額度可分次動用 → 一份契約多筆動用，額度餘額看「借款契約主檔」不看科目餘額。**', remark: '金額基礎（原文）：動用金額。🆕 v2 ·『回頭要補的』#53。⚠ 一年內到期走 `2151 短期借款`、超過一年走 `2501 長期借款`；**綜合額度可分次動用 → 一份契約多筆動用，額度餘額看「借款契約主檔」不看科目餘額。**' },
  { rule: 'LN-GUAR',    no:  1, dc: 'D', acc: '8101',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: false, optional: false, cond: '金額基礎（原文）：保證手續費。🆕 v2 ·『回頭要補的』#53。🔴🔴 **不記 `6406 銀行手續費`**——否則「實質資金成本」永遠算不出來。⚠ 新設公司無財報無擔保幾乎只能走信保基金，保證手續費 0.5–1.5% × 保證成數 80–90% → **實質年利率 2.90–3.85%，不是目標值 2.5%。**', remark: '金額基礎（原文）：保證手續費。🆕 v2 ·『回頭要補的』#53。🔴🔴 **不記 `6406 銀行手續費`**——否則「實質資金成本」永遠算不出來。⚠ 新設公司無財報無擔保幾乎只能走信保基金，保證手續費 0.5–1.5% × 保證成數 80–90% → **實質年利率 2.90–3.85%，不是目標值 2.5%。**' },
  ...
  { rule: 'LN-REPAY',   no:  1, dc: 'D', acc: '2151',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '金額基礎（原文）：本金部分。本息均攤裡的本金',                remark: '金額基礎（原文）：本金部分。本息均攤裡的本金' },
  { rule: 'LN-REPAY',   no:  2, dc: 'D', acc: '8101',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '金額基礎（原文）：利息部分。⭐ 貫穿原則：**凡資金成本一律進 `8101 利息支出`**', remark: '金額基礎（原文）：利息部分。⭐ 貫穿原則：**凡資金成本一律進 `8101 利息支出`**' },
  { rule: 'LN-REPAY',   no:  3, dc: 'C', acc: '1102',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '金額基礎（原文）：月付金額。🆕 v2。🔴 還本付息這個**行為**在資金循環（付款端），但**科目對映**放這裡與融資工具綁在一起——一筆月付要拆本金與利息，那個拆法是契約決定的。', remark: '金額基礎（原文）：月付金額。🆕 v2。🔴 還本付息這個**行為**在資金循環（付款端），但**科目對映**放這裡與融資工具綁在一起——一筆月付要拆本金與利息，那個拆法是契約決定的。' },
  { rule: 'NT-DISC',    no:  1, dc: 'D', acc: '1102',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '金額基礎（原文）：票面 － 貼現息。實得金額',                remark: '金額基礎（原文）：票面 － 貼現息。實得金額' },
  { rule: 'NT-DISC',    no:  2, dc: 'D', acc: '8101',   pattern: null,         basis: 'AMOUNT', dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '金額基礎（原文）：貼現息。⭐ 同上，資金成本一律進 8101',        remark: '金額基礎（原文）：貼現息。⭐ 同上，資金成本一律進 8101' },
  { rule: 'NT-DISC',    no:  3, dc: 'C', acc: '1112',   pattern: null,         basis: 'FACE',   dept: false, partner: true,  scope: 'PARTNER', bank: true,  optional: false, cond: '🆕 v2 ·『回頭要補的』#53。🔴 **附追索權**——票據責任沒有解除，應收票據狀態改為 `X 已貼現`（不是消失），客戶退票時銀行回頭追索。⭐⭐ 票貼買的是「時間」不是「錢」：收款等效 75 天 → 45 天，現金最低點改善 263 萬，而 24 個月貼現成本只有 23–45 萬（6–11 倍）。', remark: '🆕 v2 ·『回頭要補的』#53。🔴 **附追索權**——票據責任沒有解除，應收票據狀態改為 `X 已貼現`（不是消失），客戶退票時銀行回頭追索。⭐⭐ 票貼買的是「時間」不是「錢」：收款等效 75 天 → 45 天，現金最低點改善 263 萬，而 24 個月貼現成本只有 23–45 萬（6–11 倍）。' },
  { rule: 'NT-DISC-NG', no:  1, dc: 'D', acc: '1111',   pattern: null,         basis: 'FACE',   dept: false, partner: true,  scope: 'PARTNER', bank: true,  optional: false, cond: '帳回到該客戶的應收帳款',                           remark: '帳回到該客戶的應收帳款' },
  { rule: 'NT-DISC-NG', no:  2, dc: 'C', acc: '1102',   pattern: null,         basis: 'FACE',   dept: false, partner: false, scope: 'PARTNER', bank: true,  optional: false, cond: '🆕 v2。🔴 銀行從我方帳戶扣回——**這是「附追索權」的實質**。⚠ 接著走逾期階梯最高段（鎖貨），可能再走 `BAD` 呆帳沖銷。', remark: '🆕 v2。🔴 銀行從我方帳戶扣回——**這是「附追索權」的實質**。⚠ 接著走逾期階梯最高段（鎖貨），可能再走 `BAD` 呆帳沖銷。' },
