...
**文件狀態**：MW1 範圍（NX00 + NX99）所有決策已全部定案，詳見 §1.2 與各階段定案敘述。NX01～NX07 之 `tenant_id` 策略於各 MW 波段啟動前補齊，不影響 MW1 上線。Seed 策略已於 2026-04-03 重新定案：移除真實客戶租戶（CYTIC），改為 DEMO-LITE / DEMO-PLUS 展示租戶；真實客戶資料一律透過 CSV 匯入工具導入。**Seed 實作與驗收已於 2026-04-03 完成**（`packages/db-core/prisma/seed.ts`：平台國別／幣別、三租戶、DEMO 主檔與 nx99／nx00_view 等）。P0～P4 所有階段已於 2026-04-03 完成執行；Railway Postgres 已套用 MIG001 baseline，nx-api 已部署並通過登入驗收。MW1 完成判準達成。

**［2026-04-04 更新 — NX00 Schema 重構與雲端驗收］**（詳見 [10b-nx00-schema-refactor.md](./10b-nx00-schema-refactor.md)、[dailylog/20260404.md](../dailylog/20260404.md)）：

- **Phase 1～2**：`schema.prisma`（`Nx00BrandCodeRule`、`nx00_part.code_rule_id`、`nx99_subscription.currency_id` 等）、**MIG002**、`seed.ts`（含 **`ruleMap`**、展示料號 **`code`** 依規則組合）、**nx-api**（`/brand-code-rule`、`Part` **`codeRuleId`**）、**nx-ui** 路由與表單對齊 — **已完成並已 push `main`**。
- **Phase 3（目標庫）**：Railway（或同等目標庫）已執行 **`migrate deploy`**（含 **MIG002**）與 **`db seed`**；**2026-04-04 實測驗收通過**（含主檔讀寫、租戶隔離、UI 主檔體感）。
- **補強（與雲端驗收相關）**：**`GET /part`** 等租戶篩選與 **Lookup／audit-log** 讀取範圍、主檔 UI（明細 modal、零件關聯、原生 **`color-scheme`** 下拉）— **已併入本輪驗收通過範圍**。

**下一步**：**§銜接 MW2 前 — 雲端／正式環境驗收** 已於 2026-04-04 以實測收斂；可啟動 **§第二波段起之模組順序〔MW2+〕**，以 **NX02 庫存**為首。

### 計劃書與實務對照（2026-04-03 起）

以下為撰寫計劃當下與**實際操作**有落差處之備註，便於維運與交接對齊；細節可對照 [dailylog/20260403.md](../dailylog/20260403.md)、**2026-04-04** 起另見 [dailylog/20260404.md](../dailylog/20260404.md)（MIG002、租戶篩選、UI 與 **10b** Phase 3）。

| 主題 | 計劃書／舊稿 | 實務結果 |
|------|--------------|----------|
| **舊 migration 封存路徑** | 曾寫 `migrations/_archive/`（置於 `prisma/migrations` 下） | **正確位置**為 `packages/db-core/prisma/_archive_migrations/`（與 `prisma/migrations/README.md` 一致）。**不可**將封存目錄放在 `prisma/migrations/` 底下任一子資料夾，否則 Prisma 會把每個子資料夾當成一支 migration（P3015 等）。舊稿路徑已廢止。 |
| **前提：空庫** | 正文曾假設 Railway 尚為空庫 | **歷史前提**：專案初期確以空庫為 baseline 時機。**現況（2026-04-03）**：目標測試庫已套用 **MIG001**，見下方 **MW1 完成判準［2026-04-03 達成］**。新建環境仍適用「空庫只跑新鏈」之流程。 |
| **Railway `DATABASE_URL`（nx-api）** | §連線雙軌建議使用 Private `${{ Postgres.DATABASE_URL }}` | **目標**仍為 Private（降 Egress、延遲）。**實務**：曾發生變數引用未正確注入導致 `DATABASE_URL is missing`，改填 **Postgres Public URL 字串**後恢復；若遇相同狀況可暫用 Public 並認列成本／延遲，再改回 Private。 |
| **Railway build context／Dockerfile** | 儀表板常見「服務子目錄」與 monorepo 根目錄兩種設定 | **Dockerfile** 若自 repo 根 `COPY` lockfile、`packages/db-core`、`apps/nx-api`，則 Dashboard 宜以 **monorepo 根**為來源目錄並指定 **`apps/nx-api/Dockerfile`**；僅將 Root 設為 `/apps/nx-api` 而 context 不含根時，`COPY` 可能失敗。與「服務程式在子目錄」易混淆，設定時請對照當次 Dockerfile。 |
| **P0 檢查清單** | 階段 0 仍列未勾選 `[ ]` | 該清單為**目標／提醒項**，非「尚未執行」。連線與 DDL 煙測已於 2026-04-03 演練；重跑可見 **`pnpm run p0:railway-ddl`**（`packages/db-core`）及 dailylog。 |

**前提（歷史與現況並列）**：**歷史上**，Railway PostgreSQL 曾為**空庫**（尚未套用 migration），因而採 **P4「MW1 最小 baseline」**（見 **階段 4**）一次整併 migration 鏈。**現況**，目標環境已可視為 **MIG001 已部署**；後續波段僅疊加**標準增量 migration**。
...
- Railway Postgres 已套用 MIG001-mw1-baseline
- nx-api 已部署至 Railway（https://nx-api-production.up.railway.app）
- nx-ui 已部署至 Vercel（https://app.nexoragrid.com）
- admin 帳號登入驗收通過
- **Seed（✅ 已完成，2026-04-03）**：`prisma/seed.ts` 已對齊定案——**DEV-INNOVA**／**DEMO-LITE**／**DEMO-PLUS**；平台 **nx00_country／nx00_currency**；DEMO 租戶含完整 NX00 主檔假資料、多使用者與 **nx00_user_warehouse** 規則；**CYTIC** 不寫入 Seed；**nx99** 訂閱與 **nx00_view** 一併種子。重跑 **`pnpm exec prisma db seed`** 可 idempotent。

**［2026-04-04 追加達成］**（NX00 Schema 重構／**10b**）：

- 目標庫已可套用 **MIG002-nx00-schema-refactor**（併於 **MIG001** 之後）；**`db seed`** 與 **API／UI** 已對齊 **`brand_code_rule`**、**`Part.codeRuleId`**、**`Subscription.currencyId`** 等；**雲端實測驗收通過**（見文件開頭「［2026-04-04 更新］」與 **§銜接 MW2 前** 勾選項）。

- 下一步：**MIG002 與雲端驗收已於 2026-04-04 收斂**（見上文「［2026-04-04 更新］」與下節勾選項）；可進入 **MW2**。

---

### 銜接 MW2 前 — 雲端／正式環境驗收（必做）

**目的**：nx-api 已部署至 Railway、nx-ui 已上 Vercel，不代表 **Vercel × Railway × DB 種子 × RBAC** 全路徑無落差。**啟動第二波段（MW2）前**應完成一輪清單式驗收，避免將「僅後端 build 通過」誤等同產品可用。

**［2026-04-04］**：以下清單已於**目標雲端環境實測通過**（含 **MIG002** 後 schema／種子與 nx-api／nx-ui 現行 `main`）；後續若再變更部署或資料庫，請重跑相關項。

**建議清單（可逐項勾選）**

- [x] **`GET /health`**：`nx-api` 已提供該端點（200）。Railway 是否設定 Healthcheck Path 可選；先前部署曾留白，不影響手動驗收。
- [x] **登入與主檔讀取**：以 **DEMO `admin`** 驗 **ADMIN** 專屬寫入；以 **任一種子職務帳號**（**ADMIN／OWNER／SALES／WAREHOUSE／DRIVER／ACCOUNTANT**）驗 **使用者主檔** **`GET /user` 應 200**（依 JWT **`tenantId`** 僅看本租戶）。**變更須已部署至 Railway**；若程式仍舊，重整只會持續 **403**。
- [x] **雲端仍 403**（排查項）：先確認 **nx-api 映像已重建部署**（非僅改本機 repo）；再於瀏覽器 **Network** 檢視 **`/user`** 回應是否仍來自舊版後端。（本輪驗收未再現 403。）
- [x] **已知行為補充（2026-04-03 後）**：使用者列表／明細 **讀取** 為種子六職務；**建立／更新／啟停用** 仍僅 **ADMIN**。實作與補記見 [dailylog/20260403.md](../dailylog/20260403.md) 第 12 點（含 **USER_READ_ROLES**）。
- [x] **nx-ui 與 P1 schema 對齊**：見 [dailylog/20260403.md](../dailylog/20260403.md)（`userAccount`／`userName`、公告／行事曆、ADMIN 建立資料時 **`tenantId`** 等）；列為驗收項，避免僅後端已對齊而前端仍舊欄位或缺欄位。**2026-04-04** 另含 **10b**／**MIG002** 之 **brand-code-rule**、**`codeRuleId`**、主檔明細 UI 等，見 [dailylog/20260404.md](../dailylog/20260404.md)。
- [x] **DB 可重現**：目標庫執行 **`pnpm exec prisma migrate deploy`** 與 **`pnpm exec prisma db seed`**（於 `packages/db-core`，`DATABASE_URL` 就緒）可得到與 §Seed 一致之租戶與資料（**含 MIG002 後**之 **`nx00_brand_code_rule`**、**`nx00_part.code_rule_id`** 等）。

**通過判準（建議）**：上述項目依角色預期無誤解、無「應 200 卻 403／5xx」之未解案例；再進入 **§第二波段起之模組順序** 規劃 MW2。**［2026-04-04］已滿足，可進入 MW2 規劃與開發。**
...
| 順序 | 階段代碼 | 名稱 | 說明 | 狀態 |
|:----:|:--------:|------|------|------|
| **P0** | 階段 0 | 連線與環境（Railway） | 本機經 **Public URL** 連上 Railway Postgres、SSL／PORT 正常，並能以客戶端執行 **DDL 手測**（建表後刪除）。**不含** `migrate deploy`（併入 **P4 baseline 產出後**驗證）。 | ✅ 已完成 |
| **P1** | 階段 1 | 多租戶 `tenant_id` 策略 | **MW1 範圍已定案**（見下節 1.2）；NX01～NX07 各表仍於對應波段前補齊決策與實作。 | ✅ 已完成 |
| **P2** | 階段 2 | `schema.prisma` 中文註解與欄位順序 | 對齊 `docs/nx00_field.csv`（NX00）與既有註解慣例；方便你追加欄位與交接。 | ✅ 已完成（併入 P1） |
| **P3** | 階段 3 | Migration 代碼化與對照表 | 目錄命名帶穩定代碼、`migrations/README.md`、各 `migration.sql` 檔頭摘要。**P4 baseline 產出後**新鏈從 **MIG001** 起編碼；舊鏈封存至 `prisma/_archive_migrations/`，不改檔名（見 **應用部署與營運**）。 | ✅ 已完成 |
| **P4** | 階段 4（已定案採行） | **MW1 最小 baseline** | 空庫單包（或極少包）僅含 **nx00+nx99** 與 ID／COMMENT 等必要 SQL；舊 migration 封存。之後每個 MW **只增量**往上疊。理由見 **階段 4** 正文。 | ✅ 已完成 |
...
**檢查清單**（以下為**目標／提醒項**；2026-04-03 已演練 Public 連線與 DDL 煙測，見 §計劃書與實務對照、[dailylog/20260403.md](../dailylog/20260403.md)、`pnpm run p0:railway-ddl`。）
...
| **業務主檔**（零件、零件品牌、汽車品牌、倉庫、儲位、往來對象、零件族群、料號規則、零件關聯等） | **每租戶一套** | 料號、倉庫、客戶／供應商等均為各店自有資料，必須隔離。**於相關表新增 `tenant_id`**（含 `nx00_part`、`nx00_part_brand`、`nx00_car_brand`、`nx00_warehouse`、`nx00_location`、`nx00_partner`、`nx00_part_group`、**`nx00_brand_code_rule`**（MIG002 由舊名 `brand_code_role` 遷移）、`nx00_part_relation` 等；實作時以 schema 完整掃描補齊）。 |
...
**依賴**：**P4 baseline 產出後**於全新 migration 目錄從 **MIG001** 起代碼化；舊鏈封存至 `prisma/_archive_migrations/` 不改檔名（見 **應用部署與營運**）。
...
**舊鏈處理**：將現有 `packages/db-core/prisma/migrations/` 內歷史目錄**移出** Prisma 讀取路徑，置於 **`packages/db-core/prisma/_archive_migrations/`**（或 repo 外備份）；**勿**放在 `migrations/` 底下之子資料夾。**僅供人類查考**；全新 baseline 目錄置於 `prisma/migrations/` 下，空庫只跑新鏈。
...
- 資料庫 **無資料表**：為**專案初期**畫面（遷移前）。**現況**：目標測試庫於套用 **MIG001**、**MIG002**（**2026-04-04**）與 seed 後應已具 **nx00／nx99** 等表（含 **`nx00_brand_code_rule`**、**`nx00_part.code_rule_id`** 等）；新開空庫仍適用「先 `migrate deploy` 再 seed」順序。
- **Private Network**：後端應優先使用 `${{ Postgres.DATABASE_URL }}`。
- **Dockerfile／Root Directory**：若建置檔自 monorepo **根**複製 lockfile 與 `packages/db-core`、`apps/nx-api`，儀表板 **Source／Root** 須能讓該 Dockerfile 讀到 repo 根（常見：根目錄 + 指定 `apps/nx-api/Dockerfile`）。見 §計劃書與實務對照表「Railway build context」。
...
- Railway nx-api Variables：**目標**為 Private `${{ Postgres.DATABASE_URL }}`。**實務**：若變數引用未注入，可暫填 **Public URL 字串**（與 §計劃書與實務對照、dailylog 一致），並規劃改回 Private。
- nx-api start command **不含** `prisma migrate deploy`

### Migration 代碼化時點

P4 baseline 產出後，新目錄直接從 **MIG001** 起編碼；舊鏈封存至 `prisma/_archive_migrations/`，不改檔名，不再維護。

### Seed 範圍（已重新定案）**〔✅ 實作已完成 · 2026-04-03〕**

**Seed 目的**：建立系統初始化所需的最小資料，以及供展示用的完整假資料租戶。  
Seed 不包含任何真實客戶資料；真實客戶資料一律透過 **CSV 匯入工具**（另見任務規劃）匯入。

**實作與驗收**：已落地於 **`packages/db-core/prisma/seed.ts`**；執行 **`pnpm exec prisma db seed`**（於 `packages/db-core`，`DATABASE_URL` 就緒）。**`prisma migrate reset`** 後會依 `prisma.config.ts` 觸發種子（或手動補跑 **db seed**）。舊庫若仍殘留 **CYTIC** 租戶，請勿依賴 Seed 刪除，應以維運 SQL／手動清理（Seed 不再寫入 CYTIC）。
...
|---------|-----------|-----------|-----------|
| 租戶基本資料 | ✅ | ✅ | ✅ |
| 使用者 | ✅ 僅 admin | ✅ 10 人（含 admin） | ✅ 20 人（含 admin） |
| 角色（ADMIN／OWNER／SALES／WAREHOUSE／DRIVER／ACCOUNTANT） | ✅ 每租戶一套 | ✅ 每租戶一套 | ✅ 每租戶一套 |
| 使用者倉庫指派（規則內） | — | ✅ | ✅ |
| 廠牌／汽車品牌／族群／料號規則 | — | ✅ 含假資料 | ✅ 含假資料 |
| 倉庫／庫位 | — | ✅ 含假資料 | ✅ 含假資料 |
| 往來對象主檔 | — | ✅ 含假資料 | ✅ 含假資料 |
| 零件主檔（含關聯） | — | ✅ 含假資料 | ✅ 含假資料 |
| 首頁公告／行事曆 | — | ✅ | ✅ |
| **nx99** 方案／模組／訂閱 | ✅（PRO） | ✅（LITE） | ✅（PLUS） |
| **nx00_view** | ✅ | ✅ | ✅ |
...
7. nx00_brand_code_rule（料號規則，依賴 part_brand；舊稿曾寫 `brand_code_role`）
...
CSV 匯入工具為獨立任務；**MW1 Seed 已完成**，可啟動實作與排程。
...
- 相關程式根源：`packages/db-core/prisma/schema.prisma`、`packages/db-core/prisma/migrations/`、`packages/db-core/prisma/_archive_migrations/`（舊鏈封存，Prisma 不讀取）、`docs/nx00_field.csv`
