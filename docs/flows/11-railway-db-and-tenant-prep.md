...
以下為撰寫計劃當下與**實際操作**有落差處之備註，便於維運與交接對齊；細節可對照 [dailylog/20260403.md](../dailylog/20260403.md)。
...
- [ ] **`GET /health`**：`nx-api` 已提供該端點（200）。Railway 是否設定 Healthcheck Path 可選；先前部署曾留白，不影響手動驗收。
- [ ] **登入與主檔讀取**：以 **DEMO `admin`** 驗 **ADMIN** 專屬寫入；以 **任一種子職務帳號**（**ADMIN／OWNER／SALES／WAREHOUSE／DRIVER／ACCOUNTANT**）驗 **使用者主檔** **`GET /user` 應 200**（依 JWT **`tenantId`** 僅看本租戶）。**變更須已部署至 Railway**；若程式仍舊，重整只會持續 **403**。
- [ ] **雲端仍 403**：先確認 **nx-api 映像已重建部署**（非僅改本機 repo）；再於瀏覽器 **Network** 檢視 **`/user`** 回應是否仍來自舊版後端。
- [ ] **已知行為補充（2026-04-03 後）**：使用者列表／明細 **讀取** 為種子六職務；**建立／更新／啟停用** 仍僅 **ADMIN**。實作與補記見 [dailylog/20260403.md](../dailylog/20260403.md) 第 12 點（含 **USER_READ_ROLES**）。
- [ ] **nx-ui 與 P1 schema 對齊**：見 [dailylog/20260403.md](../dailylog/20260403.md)（`userAccount`／`userName`、公告／行事曆、ADMIN 建立資料時 **`tenantId`** 等）；列為驗收項，避免僅後端已對齊而前端仍舊欄位或缺欄位。
- [ ] **DB 可重現**：目標庫執行 **`pnpm exec prisma migrate deploy`** 與 **`pnpm exec prisma db seed`**（於 `packages/db-core`，`DATABASE_URL` 就緒）可得到與 §Seed 一致之租戶與資料。

**通過判準（建議）**：上述項目依角色預期無誤解、無「應 200 卻 403／5xx」之未解案例；再進入 **§第二波段起之模組順序** 規劃 MW2。
...
- 資料庫 **無資料表**：為**專案初期**畫面（遷移前）。**現況**：目標測試庫於套用 **MIG001** 與 seed 後應已具 **nx00／nx99** 等表；新開空庫仍適用「先 `migrate deploy` 再 seed」順序。
