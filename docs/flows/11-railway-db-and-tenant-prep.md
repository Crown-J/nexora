...
**文件狀態**：MW1 範圍（NX00 + NX99）所有決策已全部定案，詳見 §1.2 與各階段定案敘述。NX01～NX07 之 `tenant_id` 策略於各 MW 波段啟動前補齊，不影響 MW1 上線。
...
MW1 seed 僅寫入 **nx00 + nx99** 相關表，不寫入 nx01～nx07 表。各 MW 啟動前再依波段擴充 seed 範圍。
