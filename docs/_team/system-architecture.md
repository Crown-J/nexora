...
| A037 | Hank 軌 4.5 NX01-08 諮詢時揭露：`Nx01UserTeam.isLeader` 欄位 schema 已存在、但 application 層 0 使用、seed 0 紀錄、屬「欄位定義 vs 實際應用斷層」drift。Hank 諮詢拍範式 B 變體（沿用 `isLeader`、0 schema 變動、純應用層 + seed 補完）、Crown 拍軌 2 範圍擴大為「user_team / team 從零建立」+ Crown 拍 Q1~Q3「PLUS 4 leader / PRO 5 leader（LOGISTICS 不設）」業務真相 | Crown 拍軌 2 closure：(1) 新建 `apply-team.ts` template（依 nx01_department 動態建 default team、PLUS 4 / PRO 6）；(2) 新建 `apply-user-team.ts` template（role → department mapping、每部門首位 user isLeader=true、SYSADMIN/OWNER/LOGISTICS role 不 mapping）；(3) 新建 `apps/nx-api/src/shared/nx01/audience-query.service.ts` 提供 `findLeaderUserIds` / `findUserIdsByTeam` / `findUserIdsByDepartment` 三 query helper（對應 NX01-08 audience_logic enum 的 leaders_all / by_team_id / 部門 category）；(4) PrismaModule 註冊 Nx01AudienceQueryService；(5) dev DB 驗證：PLUS 4 leader / PRO 5 leader（FINANCE/HR/PRODUCT/SALES/WAREHOUSE）、LOGISTICS team 0 user（業務真相強化）；範式：依 department.code 動態 mapping（容忍 main PURCHASE vs 軌 4 PRODUCT 兩狀態、A039 PR merge 順序 robust）；TASK-A037-ISLEADER-CLOSURE-01 收斂、為軌 3 NX01-08 公告對象識別鋪路 |
| A039 | nx01_department seed code='PURCHASE' / name='採購部' vs 業界真相 drift（新性質 drift：「seed 命名 vs 業界 muscle memory」、不在 A034 規格 vs schema family、屬「設計階段未對齊目標客群行業文化」）。汽車零件業「研究產品 + 採購」同一批人 = 產品部門 = 業界本名；原 PURCHASE 對齊台灣其他產業、不對齊汽車零件業（NEXORA 目標市場）。Hank 軌 2 範圍揭露時發現規格書 NX01-08 §3.2 寫「產品部門」、schema seed 寫「採購部」、揭露給 Crown 觸發拍 ⭐ 改 schema（不是改規格書）— Q4-b 業界真相校正 | Crown 拍 Q4-b ⭐ schema seed 改、規格書不改（規格書本來寫對）；apply-department.ts code/name 改 PRODUCT/產品部；migration `20260506180000_nx01_department_rename_purchase_to_product` per-tenant backfill（dev DB PLUS+PRO 各 1 筆驗證 0 殘留）；schema.prisma + field-definitions.csv 註解範例同步；nx01_role.code='PURCHASING' 不同步改名（role / department 命名解耦、Crown 紀律）；TASK-A039-DEPARTMENT-RENAME-01 收斂、為軌 2 user_team mapping 命名鋪路 |
| A040 | A034 task 將 nx01_role.code 'PURCHASE' → 'PURCHASING' rename 後、未同步 5 個 stale 引用點（屬「migration 後應用層 / seed / docs 未同步」drift family、跟 A002/A031/A033 同模式但更危險：live system seed + production guard match 失敗風險）：(1) `apps/nx-api/src/nx02/qt/qt.controller.ts` 9 處 `@Roles('ADMIN', 'PURCHASE')` + comment + header；(2) `seed-data/system/nx01_role_view.csv` live system seed **118 處**（Hank 軌 4 揭露低估「10+」、實際遠多）；(3) `_cursorrules:231` 文件範例；(4) `docs/nx02/spec/intent/rfq-qt-api-intent.md` + `b5-impl_rfq-qt-api.md` 共 18 處（含 PURCHASE_ADMIN 假想 role 字眼）；(5) `seed-data/test/{lite,plus,pro}/nx01_user_role.csv` dead csv 共 9 處（Hank 軌 4 揭露不全、只提 pro、實際 3 個 tier 都有）。風險升級為 🔴 production guard match 失敗 | Crown 拍插隊軌 4.5 即時 closure：5 個引用點全動（純文字升級 PURCHASE → PURCHASING）；dev DB 驗證 PLUS tenant role_view PURCHASING 從 0 → 118 筆 ✅；strict 紀律守住 — 'ADMIN' role.code（A034 已改 'SYSADMIN'）stale 不擅自校正、揭露為 A042 候選；`docs/nx02/ui/po-workspace.md:136` 屬 A040 family 但 Crown 未列、揭露 + 不擅自改；TASK-A040-PURCHASE-ROLE-STALE-CLOSURE-01 收斂 |
...
| A043 | A040 family closure 漏網（Hank 軌 4.5 揭露）：`docs/nx02/ui/po-workspace.md:136` 註解 `nx01_user_role.role_code = PURCHASE` stale role 引用、屬 A040 family（PURCHASE → PURCHASING）但軌 4.5 紀律「Crown 列舉的 5 個引用點」之外、Hank 守紀律未動、揭露給 Crown 拍 | Crown 拍軌 4.7 進 A040 family 收尾、Hank 軌 4.7 grep -c 精確 1 處（對齊軌 4.5 揭露）、Edit 升級 PURCHASE → PURCHASING；TASK-A043-A045-A046-A047-LEFTOVERS-AND-CHARTER-01 commit 1 收斂 |
...
| A076 | NX01-MASTER-COMPLETE closure 揭露（協作 drift）：`nx01_site` + `nx01_warehouse` schema 已備結構化地址欄位（cityId / districtId / streetId + 6 巷弄門牌、對齊 A036 + partner.address 階段 1 並存範式）、NX01-04 地址型錄（nx01_city / district / street）資料亦已存在、但前端 site / warehouse 編輯頁目前仍是純文字 address 輸入、未接「縣市 → 區 → 街道」三級下拉 picker。屬「schema + 資料已備、UI 未接」協作 drift（跟 A036 warehouse 地址結構化同 family）。application 層已強制 address 必填、不影響資料正確性 | 🟡 中度、待 NX01-04 地址 picker 元件軌：(1) 開發三級連動下拉 picker 元件（city → district → street）；(2) 接通 site + warehouse 兩處編輯頁；(3) 順帶評估 partner 地址欄位統一範式。觸發時機：NX01-04 地址型錄 UI 軌啟動時順手接通。**不阻擋 NX01 closure** |
| A077 | NX01-MASTER-COMPLETE closure 揭露（ops 工程債）：localhost dev DB migration 已全部 apply（NX01 closure 落地驗證），但 Railway production DB `migrate status` 顯示 **67 支 pending migration**（涵蓋 NX02~NX10 全模組、非僅 NX01）；`prisma migrate deploy` 為 all-or-nothing、會一次套用全部 67 支、無法只挑 NX01 子集。closure 軌 Step 2 揭露、Crown 拍不在 closure 階段觸碰 production（dev 已驗證即達 closure 目的）| 🟡 中度 ops 工程債、**不阻擋 NX01 closure**：觸發時機 = 對齊 **TASK-RAILWAY-ENV-SPLIT**（env 拆分軌）+ **第一個真實客戶簽約前 2~4 週**、做一次完整 production `migrate deploy` + 全模組驗證；在此之前 `packages/db-core/.env` 維持 localhost、不啟用 Railway 連線（Railway 連線字串在 .env 註解區、需 Crown 當下拍板才啟用） |
