...
> ⭐ **2026-07-07 第二輪續作已完成**（見文末 §10）：早年銷貨全補 + 進貨全補 + **銷退全補**。
> 現況（本機 TW-100001）：銷貨 SO **1,620,569** / 明細 **3,058,864**；進貨 RR **117,850** / 明細 **591,799**；
> 銷退 SR **131,748** / 明細 **201,209**（soId/soItemId=null、schema 改 DB 可空、系統內仍必填）。
...
- **銷退 4**：✅ 已完成（執行長拍板 DB 可空、系統內必填、soId/soItemId=null 匯入；見 §10.5）
...
| 銷退 | SR **131,748** 張 / 明細 **201,209** 列（=抽取總數、0 跳過、soId/soItemId=null）|
...
### 10.5 銷退(4) — ✅ 已完成（執行長拍板：DB 可空、系統內必填）
- 偉盟銷退無原單參照 → **soId/soItemId 改 DB 可空**（schema.prisma + 本機 ALTER DROP NOT NULL）；
  **系統內建立銷退仍必填**（DTO `soId!`/`soItemId!` + service `assertSoReturnable` 驗證原封不動；
  另在 sales-return.service.ts 過帳/新增/編輯 3 處加防禦 guard：歷史匯入單無來源→明確擋）。
- 載入：銷退 SR **131,748** 張 / 明細 **201,209** 列（=抽取總數、0 跳過、soId/soItemId=null、
  status=POSTED 唯讀、returnMethod=S/returnReason=O/returnPolicy=S 預設）。
- 腳本：`weimeng-extract-sr.ts` / `weimeng-sr-placeholders.ts`（缺客戶0/料6/庫位6）/
  `weimeng-load-sr.ts`（吃年檔）/ `weimeng-sr-cleanup.ts` / `weimeng-sr-schema.ts` / `weimeng-overview.ts`。
- ⚠️ **執行長若有跑 nx-api dev server 需重啟**（Prisma client 已 regenerate、含 nullable 型別 + service guard）。
- 驗證：銷退列表查詢（含 customer join）12.9ms、null soId 不炸。
