...
**版本** v0.1 ｜ **撰寫** Hank ｜ **日期** 2026-08-01 ｜ ⚠ **草案・未動 schema.prisma**
...
| `reversalOfVoucherId` | FK? → self | 🔴 本張是哪一張的**紅字沖銷傳票** |
| `reversedByVoucherId` | FK? → self | 🔴 本張被哪一張沖銷了（雙向指，查任一邊都看得到）|
...
| # | 題目 | 我的建議 |
|---|---|---|
| **Q1** | 科目餘額表只帶「部門」一個維度？| ⭐ **是**。往來對象已有 AR/AP 子帳、銀行帳戶走對帳——多帶就是第二份數字，且行數乘法成長 |
| **Q2** | 試算表與財報**不建表**、由科目屬性長出來？| ⭐ **是**。A 階段已把三欄埋好；落表＝存一份會過期的複本，違反「沒有第二份數字」 |
| **Q3** | 傳票怎麼指回來源單據？| ⭐ 建議 `sourceDocType` ＋ `sourceDocId` **弱關聯**（不設 FK），需要強關聯時走既有的 `Nx98DocLink`。⛔ 不在傳票上開 20 個 FK 欄位 |
| **Q4** | 過帳顆粒度——**一張單一張傳票**，還是一天一張彙總傳票？| ⭐ 建議 **一張單一張傳票**。理由：原則 1️⃣「原子產生」；且散客零售量大時再談彙總（那是效能問題，不是設計問題）。⚠ 這題會影響傳票張數量級 |
| **Q5** | 既有子帳（`ArLedger`／`ApLedger`／`StockLedger`）與總帳的關係？| ⭐ 建議**子帳不動、加一支「驗證」**：定期比對子帳合計 vs 總帳控制科目餘額，差額不為 0 就報警。<br>⚠ 這正是原則 1️⃣ 說的「對帳從計算退化成驗證」——⛔ 但**不做自動調整**，差額必須有人看 |
