...
<!-- 檔案版本：v0.3（B1~B3 已完成並 push 2026-08-01；§5 五題已拍板） -->
...
**版本** v0.3 ｜ **撰寫** Hank ｜ **日期** 2026-08-01 ｜ ✅ **§5 五題已全數照建議拍板**（執行長 2026-08-01）
...
| B3 | — | `reverseVoucher` ＋ `discardDraftVoucher` ＋ `findReversalOf` | `21-gl-reverse-voucher.ts` 14 項 |

所有測試全程在 transaction 內、結束 rollback，DB 不留任何資料；`run-all.mjs` 已能一起跑（含 `.ts`）。

**下一步**：B4 餘額結轉（期初→本期→期末）＋ 年度結帳 `CLS`（`3202` 本期損益 → `3201` 累積盈餘）。
