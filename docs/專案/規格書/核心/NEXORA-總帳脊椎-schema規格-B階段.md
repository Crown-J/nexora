...
<!-- 檔案版本：v0.5（B1~B5 已完成並 push 2026-08-01；§5 五題已拍板） -->
...
**版本** v0.5 ｜ **撰寫** Hank ｜ **日期** 2026-08-01 ｜ ✅ **§5 五題已全數照建議拍板**（執行長 2026-08-01）
...
| B5 | — | 試算表／損益表／資產負債表／現金流量分類彙總（**純查詢、0 張新表**）| `23-gl-financial-reports.ts` 17 項 |

所有測試全程在 transaction 內、結束 rollback，DB 不留任何資料；`run-all.mjs` 已能一起跑（含 `.ts`）。

**下一步**：B6 接第一個劇本過帳點（建議從 `SO-CA` 現金銷貨起）。
