...
<!-- 檔案版本：v1.2（✅ A 階段結案 2026-08-01：NONE 維持拍板、2141 預收貨款補齊、已 push） -->
...
**版本** v1.2 ｜ **撰寫** Hank ｜ **日期** 2026-08-01 ｜ ✅ **A 階段結案**（§9 交付 / §10 v2·v11 對齊 / §11 收尾三拍板）
...
**完成判準終驗**：分錄行孤兒 **0**、指到不可記帳 **0**、指到未啟用 **0**、單邊規則 **0**（FA-TRF 無分錄除外）。

### ⚠ 結案時仍未解的（B 階段候選）

- 🔴 **`5102 進貨折讓`**：科目建了，但 `PR 進貨退出／折讓` 貸的是 `1121 存貨`，這個科目沒人用。
  要嘛把進貨退出與折讓拆成兩個代號、要嘛刪掉 `5102`——**兩者不能並存**。
- ⚠ **`2501 長期借款` 變成無人承接**：v10 的 `LN-DRAW`／`LN-REPAY` 用 `2151/2501` 樣板，
