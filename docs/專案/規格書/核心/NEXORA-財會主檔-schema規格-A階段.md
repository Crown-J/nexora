...
| 科目 | 判定 |
|---|---|
| 🔴 **`2141` 預收貨款** | **真缺口**。採購側有 `PO-PRE 預付貨款`，**銷貨側沒有對稱的「預收」交易代號**——客戶先付訂金這件事在過帳規則裡是空的。⭐ 正是亞羅自己提的對稱檢查（「有損失科目沒有利益科目就會有錢無處可放」）套在預付/預收上的結果 |
| 🔴 **`5102` 進貨折讓** | **真缺口**。`PR 進貨退出／折讓` 貸的是 `1121 存貨`，`5102` 這個科目建了卻沒人用 → 要嘛 PR 拆成退出與折讓兩碼、要嘛刪掉 5102。**兩者不能並存** |
