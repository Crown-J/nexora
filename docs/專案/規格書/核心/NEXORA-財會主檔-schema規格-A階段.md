...
<!-- 檔案版本：v1.4（✅ A 階段全數結案 2026-08-01：NONE／2141／5102／2501 四題皆已決、已 push） -->
...
**版本** v1.4 ｜ **撰寫** Hank ｜ **日期** 2026-08-01 ｜ ✅ **A 階段結案**（§9 交付 / §10 v2·v11 對齊 / §11 收尾拍板）
...
**完成判準終驗**：交易代號 **67**、分錄行 **182**、孤兒 **0**、指到不可記帳 **0**、指到未啟用 **0**、單邊規則 **0**（FA-TRF 無分錄除外）。
...
### 5️⃣ `2501 長期借款` 解掉 — **不是標預留，是規則漏了跟自己的備註對齊**

先看證據再決定，結果是 v11 自己前後矛盾：

| 出處 | 說的話 |
|---|---|
| 科目表對 `2501` 的定義 | 「銀行融資 500 萬中**一年以上部分**」 |
| `LN-DRAW` 第 2 行的**備註** | 「⚠ 一年內到期走 `2151 短期借款`、超過一年走 `2501 長期借款`」 |
| `LN-DRAW` 第 2 行的**科目欄** | ⛔ 只填了 `2151` |

→ v10 原本就是 `2151/2501` 樣板，v11 收斂科目欄時**沒跟同一列的備註對齊**。
判定為疏漏、不是刻意不做長期借款。**`2501` 保持啟用**，處置兩件：

1. `LN-DRAW` 第 2 行、`LN-REPAY` 第 1 行的科目欄**改回 `2151/2501` 分流樣板**，
   條件寫明「依借款契約到期日分流」。
2. 🆕 補 **`LN-RECL` 長期借款重分類**（借 `2501`／貸 `2151`）。

🔴 **第 2 件才是關鍵，而亞羅沒有**：只在動用當下分流是不夠的——**時間會走**。
原本掛 `2501` 的長期部分會逐漸變成「一年內到期」，不重分類的話流動負債被低估、**流動比率一路虛高**，
而流動比率正是銀行授信看的第一個數字。⚠ 每期末（至少每年結帳）跑一次，金額＝未來 12 個月內到期的本金。

⚠ **依賴**：分流要判到期日，得有「借款契約主檔」（亞羅待補 #49、尚未建）。
A 階段先把規則宣告好，資料源留給融資軌。

交易代號 **66 → 67**、分錄行 **180 → 182**。

### ⚠ 結案時仍未解的（B 階段候選）

- `7101 利息收入`／`7102 其他收入`／`8103 其他損失`：雜項性質，人工傳票選用即可，合理。
  **這是全部剩下的 3 個，且都有交代。**
- ⚠ **過帳規則尚未綁單據**：`sourceDocType` 全欄為 null。把 67 個代號接到實際單據（`SO`／`RR`／`AL`…）
  屬 B 階段的 service 工作，不是主檔層。
- ⚠ **借款契約主檔未建**（亞羅待補 #49）：`LN-*` 那幾條的到期日分流、綜合額度分次動用都要靠它。屬融資軌。
