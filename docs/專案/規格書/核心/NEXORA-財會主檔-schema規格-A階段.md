...
<!-- 檔案版本：v1.3（✅ A 階段結案 2026-08-01：NONE 維持、2141 預收貨款＋5102 進貨折讓皆補齊、已 push） -->
...
**版本** v1.3 ｜ **撰寫** Hank ｜ **日期** 2026-08-01 ｜ ✅ **A 階段結案**（§9 交付 / §10 v2·v11 對齊 / §11 收尾拍板）
...
## 9. ✅ A 階段交付紀錄（2026-08-01 完成）

| commit | 內容 |
|---|---|
| `e4d53728` | 規格 v0.1 草案 |
| `4352b00b` | 規格 v0.2：七題拍板 ＋ 三個 grep 實測 |
| `1b00e388` | schema.prisma：18 新 model ＋ 3 表擴欄（192 → 210） |
| `8b6e6a97` | migration SQL 810 行（純 additive） |
| `302a8ca1` | 套用本機 ＋ 欄位備註同步 |
| `264816f5` | seed 全套 9 支 ＋ 恆迎租戶實跑驗證 |

### 實測數字（非估算）

| 項目 | 數 |
|---|---|
| 新表 / 新 ID 產生器 / 新外鍵 | 18 / 18 / 45 |
| 會計科目 | 109（可記帳 86、標題 23）|
| 交易代號 / 分錄行 | **63 / 169** |
| 稅別 / 收付方式 / 資產類別 / 付款條件 / 參數 / 會計政策 | 7 / 6 / 5 / 8 範本 / 34 筆 9 類 / 14 項 |

### ✅ 完成判準（§8 定的三條，逐條實測）

1. **63 個交易代號全部查得到有效科目** → 169 分錄行 ＝ 152 固定科目 ＋ 17 樣板；
   孤兒 0、指到不可記帳 0、指到已停用 0。
2. **關帳四閘欄位就位**（`gateBankRecDiff` / `gatePettyCashDiff` / `gateLandedCostBalance` / `gateAssetDiff` ＋ `canClose`）——算法屬 B 階段 service。
3. **資產可算出每月折舊** → `Nx05Asset`（成本／殘值／耐用年數／起算月）＋ `Nx05AssetDepreciation`（逐月一列、`@@unique([assetId, periodCode])`）就位。

### 🔴 轉譯時對亞羅原始資料做的 3 個修正

| # | 修正 | 依據 |
|---|---|---|
| 1 | **對映表筆誤 `2134` → `2121`** | FA-DISP 第 4 行寫 `2134` 標「銷項稅額」，但科目表的銷項稅額是 `2121`。⚠ **這正是該表自己警告過的恆迎病**——「恆迎的對映表與科目主檔對不上，1131 在兩張表有兩種定義」。亞羅犯了同一個錯 |
| 2 | **補建 4 個科目** `2115`／`2116`／`6411`／`7103` | 不是我想加——是**對映表實際引用、而科目表沒有**（PAY-LV／BON-P·BON-U／FA-EXP／FA-DISP）。跑驗證才浮出來 |
| 3 | **`1901 開辦費` 建但停用** | 現行商業會計處理準則已刪除其遞延資產屬性。停用而非刪除＝留痕、UI 不列、待記帳士確認 |

### ⚠ 新發現的規格缺口（A 階段沒解，列 B 階段候選）

實測「有哪些可記帳科目沒有任何交易代號承接」，扣掉被 `6xxx`／`15xx` 樣板涵蓋的，**剩 5 個**：

| 科目 | 判定 |
|---|---|
| 🔴 **`2141` 預收貨款** | **真缺口**。採購側有 `PO-PRE 預付貨款`，**銷貨側沒有對稱的「預收」交易代號**——客戶先付訂金這件事在過帳規則裡是空的。⭐ 正是亞羅自己提的對稱檢查（「有損失科目沒有利益科目就會有錢無處可放」）套在預付/預收上的結果<br>✅ **已於 2026-08-01 解決 → 見 §11 2️⃣：補建 `SO-PRE` ＋ `SO-DLV` 一對** |
| 🔴 **`5102` 進貨折讓** | **真缺口**。`PR 進貨退出／折讓` 貸的是 `1121 存貨`，`5102` 這個科目建了卻沒人用 → 要嘛 PR 拆成退出與折讓兩碼、要嘛刪掉 5102。**兩者不能並存**<br>✅ **已於 2026-08-01 解決 → 見 §11 4️⃣：採「拆成兩碼」（新增 `PD` 進貨折讓、`PR` 正名進貨退出）** |
| `7101` 利息收入 / `7102` 其他收入 / `8103` 其他損失 | 合理：雜項性質，人工傳票選用即可 |

### ⚠ 順帶記錄

- **既有 drift 164 行**（index 命名慣例 `_fkidx` vs `_idx`、permission_level 約束名等）與本軌無關，但存在已久，值得另開一軌收斂。
- Prisma 7 CLI 變更：`db execute` 不再吃 `--schema`（改讀 `prisma.config.ts`）、`migrate diff` 的 `--from-schema-datamodel` 改名 `--from-schema`。

---

## 10. ⚠ 來源升版對齊：亞羅 v1/v10 → v2/v11（2026-08-01 當日）

A 階段實作進行到一半時，執行長 commit `92aebce1` 帶進亞羅 **v2/v11**（本檔 §0 引用的是 v1/v10）。
逐項比對後**整套 seed 已改抓 v2/v11 重產並重跑**。差異如下：

### 會計科目 105 → 113（+8）

| 新增 | 內容 |
|---|---|
| ⭐ **4 個與我獨立推斷完全一致** | `2115 應付特休假薪資`／`2116 應付獎金`／`6411 什項購置`／`7103 財產交易利益`——我是**從對映表引用了卻查不到科目**反推出來的，亞羅是從自己的待補清單補的，**兩條路得到同一組答案** |
| 3 個投資擴充位 | `1151 短期投資`／`16 長期投資`／`1601 長期股權投資`，狀態 **⛔ 預留（不啟用）**——決策 1️⃣「範圍砍掉 ≠ 架構焊死」的配套 |
| 1 個 | `3102 資本公積`（因 `EQ-IN` 的溢價部分改記這裡）|

⚠ `1901 開辦費` 在 v2 仍保留。本 repo 維持**建但停用**的處置（比來源更保守，待記帳士確認）。

### 交易代號 63 → 63（不變）、分錄行 169 → 169（不變），但 **7 行內容有變**

`LN-DRAW`／`LN-REPAY` 的 `2151/2501` 樣板收斂成 `2151` 單一科目、`EQ-IN` 溢價改記 `3102`、
`NT-DISC` 改「票面 − 貼現息」、`NT-DISC-NG` 第 2 行改記 `1102`、`LN-GUAR`／`LN-REPAY` 金額基礎修文字。

### 🔴 循環映射：v11 的 27 行映射表推翻了我的 6 個標法

v11 新增『資金－法定循環映射』分頁，是決策 3️⃣ 的權威落地。對照後修正：

| 交易代號 | 我原本標 | 依 v11 改為 | 依據 |
|---|---|---|---|
| `EMP-EXP` 員工代墊 | `PAYROLL` | **`PURCHASE_PAYMENT`** | 第 ⑳ 行明講：對手方是員工但性質是墊付的營運費用、不是勞務對價 → 不歸薪工 |
| `WHT` 扣繳與申報 | `TREASURY` | **`PAYROLL`** | 第 ㉒ 行：`2113`/`2114` 來源是薪資發放。⭐ 這是拍板時未預見的第三條 |
| `BK-TRF` 軋票撥款 | `TREASURY` | **`PURCHASE_PAYMENT`**（＋備註標「支援作業・硬塞」）| 第 ⑫ 行：如實標示、⛔ 不假裝它是採購付款的一環 |
| `OFFS` 抵帳 | `PURCHASE_PAYMENT` | **`DUAL_SALES_PURCHASE`** | 第 ⑮ 行：27 行裡唯一同時屬兩條 |
| `VAT`／`TAX-PP`／`TAX-FS`／`CLS` | `TREASURY` | **`NONE`** | 第 ㉑㉓ 行：⛔ 法定九大無對應 |
| `PC-ADV`／`PC-EXP`／`FX` | `TREASURY` | **`PURCHASE_PAYMENT`** | 第 ⑱⑲ 行（零用金屬費用端）；`FX` 來自採購＋資金 |

⚠ **這與 Q7 拍板的字面（「標 `TREASURY`」）不同、但與它的用意一致**——Q7 的用意是「老實標、不硬塞進九大」。
v11 逐行標完後給出更精確的答案：那幾行**根本沒有法定的家**，所以誠實的標法是 `NONE` 而不是自創一條。
🔴 **這一項請執行長確認**：要維持 `NONE`（忠於 v11 映射表），還是回到 Q7 字面的 `TREASURY`？

### ⭐ v11 順帶給出一條可複用判準（已抄進本檔備查）

> **一條「內部循環」值不值得獨立，看它有多少行為在法定框架下無家可歸。**
> 人資→薪工 31 行全對得上、商品開發→研發 15 行全對得上 → 寫在名字括號裡就夠；
> 資金 **22.2%（6/27）無家可歸、且無家可歸的剛好是價值最高的三個**（銀行對帳、13 週現金預測、月結複核）→ 必須獨立。

### 實作後最終數字（v2/v11 版）

科目類別 8 ／ 稅別 7 ／ **會計科目 113（可記帳 89）** ／ 收付方式 6 ／ 資產類別 5 ／
付款條件 8 範本 10 行 ／ **過帳規則 63 代號 169 行** ／ 參數 34 筆 9 類 ／ 會計政策 14 項 1 例外。
完成判準重驗：分錄行孤兒 **0**、指到不可記帳 **0**、指到已停用 **0**。

---

## 11. ✅ 收尾三拍板（2026-08-01・執行長）

### 1️⃣ `NONE` 維持

§10 提的那題定案：`VAT`／`TAX-PP`／`TAX-FS`／`CLS` 的法定循環維持標 **`NONE`**（法定九大無對應），
不回到 Q7 字面的 `TREASURY`。

⭐ **理由留痕**：Q7 的用意是「老實標、不硬塞進九大」。亞羅 v11 逐行標完 27 個行為後給出更精確的答案——
那幾行**根本沒有法定的家**（22.2% 無家可歸），所以誠實的標法是 `NONE`，而不是自創一條循環硬塞。
⚠ 對外做 COSO 檢視時，這幾條要放在「內部控制之監督」或「跨循環作業」那一節說明，不掛在任何一條循環底下。

### 2️⃣ `2141 預收貨款` 補齊 — 補的是**一對**不是一個

⚠ 只補「收訂金」而不補「沖銷」，`2141` 只會進不會出、餘額永遠對不平。
形狀完全對稱採購側既有的 `PO-PRE`（預付貨款）／`PO-RCV`（到貨・預付轉存貨）：

| 新代號 | 分錄 | 說明 |
|---|---|---|
| **`SO-PRE`** 預收貨款（收款段）| 借 `1102` 銀行存款／貸 `2141` 預收貨款 | 錢先進來、貨還沒出。現金收訖走 `1101` |
| **`SO-DLV`** 出貨（預收轉收入）| 借 `2141` 已收部分 ＋ 借 `1111` 未收部分*<br>貸 `4101` 未稅 ＋ 貸 `2121` 稅額<br>借 `5101`／貸 `1121` 結轉成本 | *尾款那行標 `isOptional`（訂金未涵蓋全額時才出現）|

🔴 **⚠ 一個業務語意假設要記帳士確認**：本規則比照亞羅 `PO-PRE` 的處置——
「付訂金通常沒有發票，所以進項稅額不在這一段」→ 收訂金亦不開發票，**銷項稅額掛在出貨那一段**。
若實務上收訂金即須開立統一發票，`SO-PRE` 要拆出 `2121 銷項稅額` 一行。

交易代號 **63 → 65**、分錄行 **169 → 177**。

### 3️⃣ 已 push

⚠ 本軌 10 個 commit 已推上遠端（`main`）。
...
| # | 問題 | 影響 |
|---|---|---|
| 1 | 🔴 **啟用判斷式只認「停用」** | v2 的 3 個投資擴充位狀態是「⛔ 預留（不啟用）」，被誤建成**啟用**——**這剛好違背決策 1️⃣「留位但不啟用」的整個重點**。已改為「停用／預留／不啟用」三者皆視為不啟用 |
| 2 | 補建規則的借貸沒轉碼 | `SO-PRE`／`SO-DLV` 的 `drCr` 直接寫成中文「借／貸」而非 `D`／`C`（中文轉碼只發生在讀 xlsx 那一段）。已修 |
| 3 | `condition` 被無條件塞成備註的複本 | 169 條分錄行的 `condition` 全部等於 `remark`，等於這個欄位沒有意義。已改為**只在真的有條件時才填**（借貸二選一／該行條件性出現／金額基礎需要原文說明）|

⭐ 順帶再一次殊途同歸：v2 對 `1901 開辦費` 標的是「⛔ 停用（待記帳士確認刪除）」，
與本 repo 原本獨立做的「建但停用」處置完全一致 → 已移除我方覆寫，忠於來源。

### 最終數字（結案）

| 項目 | 數 |
|---|---|
| 新表 / ID 產生器 / 外鍵 | 18 / 18 / 45 |
| 會計科目 | **113**（可記帳 89、其中 4 個停用或預留）|
| 交易代號 / 分錄行 | **66 / 180** |
| 稅別 7 ／收付方式 6 ／資產類別 5 ／付款條件 8 範本 10 行 ／參數 34 筆 9 類 ／會計政策 14 項 1 例外 | — |

**完成判準終驗**：交易代號 **66**、分錄行 **180**、孤兒 **0**、指到不可記帳 **0**、指到未啟用 **0**、單邊規則 **0**（FA-TRF 無分錄除外）。

### 4️⃣ `5102 進貨折讓` 解掉 — 答案是**拆代號**，不是刪科目

先查了才下判斷，結果是進貨側與銷貨側**不對稱**：

| 側 | 貨真的退回（動存貨）| 只降價（不動存貨）|
|---|---|---|
| 銷貨 | `SR` 銷貨退回 → 借 `4102`、且 借 `1121`／貸 `5101` 貨退回入庫 | `SD` 銷貨折讓 → 借 `4103`，**存貨不動** |
| 進貨（原） | ←　`PR 進貨退出／折讓` **一個代號包兩件事**　→ | ⛔ 沒有落點 |
| 進貨（現） | `PR` **進貨退出** → 貸 `1121` 存貨 | 🆕 `PD` **進貨折讓** → 貸 `5102`，**存貨不動** |

⭐ **對稱依據就在亞羅自己身上**：它的銷貨側早就把「退回」與「折讓」拆成兩個代號、兩個科目（`4102`／`4103`），
進貨側卻壓在一起，而那條規則只長了「退出」的形狀（貸存貨）→ `5102` 因此建了沒人用。照銷貨側的形狀拆開即可。

📌 **而且 NEXORA 這邊的單據早就分好了**：`nx05_allowance.allowance_type` 的 `P`＝進貨折讓（廠商給我方）、
`S`＝銷貨折讓（我方給客戶）。**單據有、過帳規則沒有**——缺的一直只是這一條規則，不是科目多餘。

- 🆕 `PD` 進貨折讓：借 `2101` 含稅／貸 `5102` 未稅／貸 `1133` 稅額
- ⚠ `PR` 正名為 **進貨退出**（原名「進貨退出／折讓」把兩件事壓在一個代號）

交易代號 **65 → 66**、分錄行 **177 → 180**。

### ⚠ 結案時仍未解的（B 階段候選）

- ⚠ **`2501 長期借款` 無人承接**：v10 的 `LN-DRAW`／`LN-REPAY` 用 `2151/2501` 樣板，
  v11 收斂成 `2151` 單一科目 → `2501` 從此沒有交易代號。**若融資確實只做短期，`2501` 應標預留**；
  若會有一年以上的借款，`LN-DRAW`／`LN-REPAY` 要能依到期日分流兩個科目。⚠ 這題屬融資循環，本軌不決。
- `7101 利息收入`／`7102 其他收入`／`8103 其他損失`：雜項性質，人工傳票選用即可，合理。
- ⚠ **過帳規則尚未綁單據**：`sourceDocType` 全欄為 null。把 66 個代號接到實際單據（`SO`／`RR`／`AL`…）
  屬 B 階段的 service 工作，不是主檔層。
