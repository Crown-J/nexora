<!-- NEXORA - NX01 - 共用基礎模組需求規格書 v1.1 -->

# NEXORA - NX01 - 共用基礎模組需求規格書

> 文件版本：v1.1（Crown 拍 NX01-03 拆 partner / 地址 2 份、編號全 +1）
> 最後更新：2026-05-04
> 撰寫者：Alex（Claude PM AI）
> 審核者：Crown Lin
> 狀態：v1.1 正式版、進「子規格書展開」階段（NX01-03 / NX01-04 起）
...
### 7.2 子規格書清單（v1.1 拍板版、共 17 份）

對齊 Q3 拍板「合併型錄類」+ Q4 拍板「partner 1 份通用」+ v1.1 update「partner / 地址拆 2 份」：

| 編號 | 子規格書名稱 | 階段 | 字數預估 |
|------|------------|------|---------|
| NX01-01 | 用戶管理工作站 | A | 3000~5000 |
| NX01-02 | 角色權限工作站 | A | 3000~5000 |
| NX01-03 | 客戶/供應商主檔工作站（純 partner、5 種 partner_type 通用）| A | 6000~8000 |
| NX01-04 | 客戶地址管理子模組（billing_address + shipping_address）| B | 5000~7000 |
| NX01-05 | 料號主檔工作站（含品牌編碼 + 注音搜尋）| A+B | 6000~9000 |
| NX01-06 | 倉庫主檔 | A | 3000~5000 |
| NX01-07 | 基礎型錄管理(part_brand / warehouse_type / customer_grade / currency / part_group 5 個合併)| A+B | 6000~8000 |
| NX01-08 | 公告系統 | A | 3000~5000 |
| NX01-09 | 地址型錄系統（city / district / street）| B | 4000~6000 |
| NX01-10 | 注音快搜系統（dictionary / index）| B | 4000~6000 |
| NX01-11 | 品牌編碼規則 | B | 3000~5000 |
| NX01-12 | 汽車品牌型錄（car_brand）| C | 2000~3000 |
| NX01-13 | 車型基本資料（model）| C | 4000~6000 |
| NX01-14 | 引擎資料（engine）| C | 3000~5000 |
| NX01-15 | 車輛分類型錄（transmission / drivetrain / model_type 3 個合併）| C | 4000~6000 |
| NX01-16 | 零件車型關聯（part_model）⭐ | C | 5000~7000 |
| NX01-17 | 零件版本 + 關聯（part_version + part_relation 合併）| C | 4000~6000 |

合計 17 份子規格書、預估 70000~106000 字。

**v1.1 變更說明：**

原 v1.0 NX01-03「客戶/供應商主檔工作站」涵蓋「partner + 地址 + 注音搜尋」過於龐大（8000~12000 字、超出 spec-template 範本密度）、Crown 拍 v1.1 拆成 2 份：
- NX01-03（純 partner、5 種 type 通用）
- NX01-04（地址管理子模組、billing + shipping）

原 NX01-04 ~ NX01-16 編號全 +1（規劃階段改編號、未 push GitHub、成本低）。
...
v1.0 = 階段 A（已實作）+ 階段 B 完成
       10 份子規格書（NX01-01 ~ NX01-11）
       業務跑得起來、對齊 multi-Hank 試跑時程
       
v2.0 = 階段 C 完成（再加 6 份 = 共 16 份）
       戰略表完整、對齊 Yaro 2028 開業
       
v3.0 = 階段 D 完成（再加 8 份 = 共 24 份）
       輔助主檔完整、對齊長期業務擴張
```
...
| v1.1 | 2026-05-04 | Alex | Crown 拍 NX01-03 拆 partner / 地址 2 份、編號全 +1（16 → 17 份子規格書）|
...
NX01-03 子規格書 scope 拆分:
- 原 v1.0 NX01-03 涵蓋「partner + 地址 + 注音搜尋」、字數預估 8000~12000、超出 spec-template 範本密度
- v1.1 拆成 2 份:
  - NX01-03 客戶/供應商主檔工作站（純 partner、5 種 type 通用、6000~8000 字）
  - NX01-04 客戶地址管理子模組（billing + shipping、5000~7000 字）
- 原 NX01-04 ~ NX01-16 編號全 +1
- 子規格書總數 16 → 17 份
- §7.2 / §7.4 同步 update
